...
# Task-independent structured bootstrap contract.
...
index = replace_once(index,
    ...
    'followup guidance')
index = replace_once(index, "                  const result = await def.execute(args, exec)\n",
...
for old_en in old_en_candidates:
    if old_en in client:
        client = client.replace(old_en, new_en, 1)
        break
else:
    print('warning: English hint shape not recognized; leaving it unchanged')
