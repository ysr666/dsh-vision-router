...
export function applyVisionRuntimeComposition(ctx, config = {}, core) {
  // Runtime i18n is an adapter for the mature Core only. Composition-owned
  // settings, routing, capability and browser layers continue to see the native
  // Host context and the unprojected Core contract.
  ...
  // index. The i18n facade is used here only because guard-stop surface repair
  // calls planGuardStopShadows before Core.apply. Every other helper continues
  // to delegate byte-for-byte through the facade.
