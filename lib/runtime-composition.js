...
export function applyVisionRuntimeComposition(ctx, config = {}, core) {
  ...
  // Ground/detect own one explicit raster-coordinate boundary. It sits at the
  // final Core registration edge, after Host/runtime policy composition, and
  // receives the already-created canonical SessionVisionIndex. Provider
  // fallback/retry/budget semantics remain in mature Core; only the image frame
  // and source-coordinate publication are transformed here.
  const groundingCoordinateCtx = contextWithGroundingCoordinateFrame(backendRuntimeCtx, {
    core,
    config: runtimeConfig,
