...
export function applyVisionRuntimeComposition(ctx, config = {}, core) {
  ...
  // Ground/detect own one explicit raster-coordinate boundary. Put it inside
  // the final backend policy so Core still receives backendRuntimeCtx exactly;
  // the backend policy remains the outermost Core-visible runtime boundary.
  // Provider fallback/retry/budget semantics stay in mature Core while the
  // coordinate wrapper owns only the model raster and source-pixel publication.
  const groundingCoordinateCtx = contextWithGroundingCoordinateFrame(performanceCtx, {
    core,
    config: runtimeConfig,
  ...
  const backendRuntimeCtx = contextWithVisionBackendRuntimePolicy(groundingCoordinateCtx, {
    config: runtimeConfig,
    core,
    evidenceSource: (provider, model) => liveDiscovery.evidenceSource?.(provider, model),
    logger: logging.logger,
