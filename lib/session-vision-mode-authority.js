...
/** Persist the immutable authority captured for one already-assembling step. */
...
/** Drop a completed/abandoned step snapshot before the next prompt assembly. */
export function clearSessionVisionModeAuthorityForAgent(agent) {
  if (isWeakKey(agent)) agentAuthorities.delete(agent)
}

/**
 * Execution-time fail-closed check. Real Agent steps use the prompt-assembly
 * snapshot, so a concurrent picker change applies to the next assembled step
 * instead of tearing the current request in half. Direct/internal calls with an
 * Agent but no captured assembly resolve the live DSH selection synchronously.
 */
