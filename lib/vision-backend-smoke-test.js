...
export function normalizeVisionSmokeSelection(value) {
  ...
  const provider = boundedText(value.provider, PROVIDER_MAX)
  const model = boundedText(value.model, MODEL_MAX)
  if (provider === '' || model === '') return undefined
  if (provider.includes('\0') || model.includes('\0')) return undefined
...
function verifyProbeOutput(output) {
  ...
  return digits === VISION_BACKEND_SMOKE_TEST_CODE
}
...
export async function runExactVisionBackendSmokeTest({
  ...
  return {
    ...
    fallbackUsed: false,
    imageRequest: true,
    verified: verifyProbeOutput(text),
    provider: selection.provider,
    model: selection.model,
    transport,
    latencyMs,
