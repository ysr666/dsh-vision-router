...
export function routePostBootstrapScene(baseline = {}) {
  const visualKind = textOf(baseline?.visual_kind).toLowerCase()
  ...
  if (visualKind === 'ui') addSignal(bucket, 'ui', 1.05, 'bootstrap visual_kind=ui')
  if (visualKind === 'general') addSignal(bucket, 'other', 0.9, 'bootstrap visual_kind=general')
  if (visualKind === 'document') addSignal(bucket, 'document.table', 0.2, 'bootstrap visual_kind=document')
