...
function wrapSettingsContext(ctx) {
  if (!ctx || (typeof ctx !== 'object' && typeof ctx !== 'function') || !ctx.settings) return ctx
  const settings = wrapSettingsService(ctx.settings)
        ...
        return register.call(target, {
          ...
          async execute(args, exec) {
            ...
            return toolRuntime.run(
              state,
              () => withMergedTurnSignal(
                state.signal,
                () => executeAbortable(state.signal, () => execute(args, exec)),
              ),
            )
          },
        })
 ...
 *   when older core helpers omit it explicitly, and tool promises release
      ...
      if (property === 'inject') {
        ...
        return (dependencies, callback, ...rest) => {
          ...
          return inject.call(
            target,
            dependencies,
            (childCtx) => callback(wrapSettingsContext(childCtx)),
            ...rest,
          )
        }
      }
