...
async function executeAbortable(signal, execute) {
  ...
  try {
    // A libvips operation already executing in native code cannot be forcibly
    // terminated from JS. This race still releases the agent immediately; the
    // ambient signal remains attached to the detached continuation so later fs
    // reads/resource waits/artifact publication can fail closed.
...
function stateFor(name, exec) {
  return {
    ...
    // The outer live cache owns every describe result. Core's legacy cache is
    // suppressed for describe calls so hot TTL/count changes take effect now,
    // and ordered multi-image inputs cannot alias after core sorts content ids.
      ...
      if (property === 'watch') {
        ...
        return (callback, ...rest) => watch.call(target, (...args) => {
          try {
            ...
            // A broken diagnostics read must not interfere with Host delivery.
          }
      ...
      if (property === 'register') {
        ...
        return (namespace, ...args) => {
          ...
          try {
            ...
            // Core can still run from boot config if the settings service is late.
          }
...
function wrapSettingsContext(ctx, runtimeState) {
  ...
  return new Proxy(ctx, {
    get(target, property) {
      ...
      // Settings-only proxy: every lifecycle method is still bound to the
      // exact injected child. Never proxy unrelated webServer/attachment child
      // contexts; rc.6 uses their identity for route/effect ownership.
      const value = Reflect.get(target, property, target)
      return typeof value === 'function' ? value.bind(target) : value
    },
  })
 ...
 *
 * - Relative fs paths resolve against the active session cwd.
 * - exec.signal is inherited by fs reads/resource waits and tool promises
 *   release promptly when the user cancels.
 * - each invocation receives a unique artifact run id.
 * - vision_describe uses an order-sensitive cache whose limits/TTL are live.
 * - direct HTTP endpoint edits rotate only the internal breaker identity.
 * - replaced proxy dispatchers are gracefully closed instead of accumulating.
 */
      ...
      if (property === 'inject') {
        ...
        return (dependencies, callback, ...rest) => {
          ...
          return inject.call(
            target,
            dependencies,
            (childCtx) => callback(wrapSettingsContext(childCtx, runtimeState)),
            ...rest,
          )
        }
      }
