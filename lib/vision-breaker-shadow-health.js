...
function legacyTurnNumberOf(session) {
  try {
    ...
    // (session-vision-index.js) so the legacy fallback still resolves a turn
    // on those Hosts instead of pinning every scope to ":0".
...
export function visionBreakerScopeOf(session, turnOf) {
  let turn
  try {
    if (typeof turnOf === 'function') turn = turnOf(session)
  } catch {
    turn = undefined
  }
  if (!Number.isInteger(turn) || turn < 0) turn = legacyTurnNumberOf(session)
  return `${sessionIdOf(session)}:${turn}`
}
 ...
 */
export function createVisionBreakerShadowHealth(ctx, options = {}) {
  const sessionTurnResolver = options.sessionTurnResolver ?? createSessionTurnResolver(ctx)
  const turnOf = sessionTurnResolver.turnOf
  let breaker
  return {
    capture(value) {
    ...
    async healthForCandidate(candidate, context = {}) {
      ...
      const scope = visionBreakerScopeOf(context.session, turnOf)
