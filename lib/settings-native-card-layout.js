...
export function transformSettingsIaToNativeCards(source = SETTINGS_IA_CLIENT_PRELUDE) {
  ...
  next = replaceRequired(
    ...
    `      var valid=validChain(chainRows())&&validWrapperRows(wrapperRows())&&validGuidance(guidanceRows())&&invalidKeys.length===0;
      ...
      function pageDirty(id){return pagePlan(id).length>0||invalidKeys.some(function(key){return pageOwns(id,key);});}
  ...
  next = replaceRequired(
    ...
    "h('button',{type:'button',className:'vr-reset',disabled:dirty||!writable||saving,onClick:function(){void resetField(key);}},tx('恢复默认','Reset'))",
  ...
  const newRender = `      var pages=[
      ...
      function pageCard(item){
        var id=item[0],opened=page===id,pending=pageDirty(id),cardValid=pageValid(id);
          ...
          opened?h('div',{className:'vr-ia-plugin-card-body'},
            ...
            id==='diagnostics'?null:h('div',{className:'vr-ia-card-footer'},
              ...
              h('div',{className:'vr-ia-save-actions'},
                h('button',{type:'button',className:'vr-btn',disabled:!pending||saving,onClick:function(){discardPage(id);}},tx('放弃','Discard')),
                h('button',{type:'button',className:'vr-btn vr-btn-save',disabled:!pending||!cardValid||!writable||saving,onClick:function(){void savePage(id);}},pageState.status==='saving'?tx('保存中…','Saving…'):tx('保存','Save'))
              )
            )
          ):null
        );
      ...
      return h('div',{className:'vr-settings-ia-root','data-vr-settings-ia':'1','data-vr-dirty':dirty?'1':'0','data-vr-invalid':valid?'0':'1'},
        h('div',{className:'vr-ia-page-head'},
          ...
          h('button',{type:'button',className:'vr-btn vr-ia-guide-button',title:dirty?tx('请先保存或放弃当前修改','Save or discard current changes first'):'',disabled:dirty||typeof helpers.startVisionSettingsGuide!=='function',onClick:replayGuide},tx('重新查看新手引导','Show beginner guide again'))
        ),
