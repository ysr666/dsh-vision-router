...
export const EXACT_VISION_TEST_CLIENT = String.raw`(function(){
  ...
  function makeControl(row){
    ...
    status.textContent=text('快速自检 · 1次请求 · 只验证当前模型能否识图 · 不走备用','Quick check · 1 request · verifies image input only · no fallback');

    var button=document.createElement('button');
    button.type='button';
    button.setAttribute('data-vr-exact-vision-button','1');
    button.textContent=text('测试识图','Test vision');
    button.title=text('快速自检：只验证当前模型是否能正确接收并识别图片，不生成Auto能力评分。','Quick check: verifies this exact model can receive and recognize an image; it does not create Auto capability scores.');
  ...
  async function run(row,control){
    ...
    setStatus(control,text('正在做快速识图自检；仅发送1张内置测试图，不会调用备用模型','Running quick image check; sending one built-in image to this exact model only'));
