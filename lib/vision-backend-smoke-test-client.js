...
export const EXACT_VISION_TEST_CLIENT = String.raw`(function(){
  ...
  function removeExactTestControls(){
    var controls=[];
    try { controls=Array.prototype.slice.call(document.querySelectorAll('['+CONTROL_ATTR+']')); }
    catch (_) { controls=[]; }
    controls.forEach(function(control){
      try { control && control.remove && control.remove(); }
      catch (_) {}
    });
  }
  ...
  async function run(row,control){
    ...
    try {
      ...
      if (body.verified === true) {
        setStatus(control,'✓ '+selection.provider+'/'+selection.model+text(' · 图片识别正常',' · image verified')+(latency?' · '+latency:''),body.output || '');
      } else {
        setStatus(
          control,
          '⚠ '+selection.provider+'/'+selection.model+text(' · 请求成功，但测试图识别不一致',' · request succeeded, image check mismatched')+(latency?' · '+latency:''),
          (body.output || '') + text('；可能未真正处理图片','; the endpoint may not actually process image input')
        );
      }
    } catch (error) {
  ...
  function scan(){
    if (!localPage()) return;
    if (v2OwnsCapabilityTesting()) {
      removeExactTestControls();
      return;
    }
    var rows=[];
