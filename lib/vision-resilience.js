...
export function createVisionTurnMemory({
  ...
  const evidenceLimit = Math.max(1, Math.floor(Number(maxEvidencePerScope) || 32))
  ...
  return {
    ...
    resolveSource(scope, sourceKey) {
      const key = typeof sourceKey === 'string' ? sourceKey : ''
      const hit = scopes.get(scope)
      if (hit === undefined || key === '') return key
      for (const [artifactToken, rootSource] of hit.derivedArtifacts) {
        if (key === artifactToken || key.includes(artifactToken)) return rootSource
      }
      return key
    },

    recordLocalOcr(scope, sourceKey, evidence = {}) {
      const hit = entry(scope)
      const key = this.resolveSource(scope, sourceKey)
      if (key === '') return
      hit.localOcr.delete(key)
      hit.localOcr.set(key, Object.freeze({ uncertain: evidence.uncertain === true }))
      while (hit.localOcr.size > evidenceLimit) hit.localOcr.delete(hit.localOcr.keys().next().value)
    },

    hasLocalOcr(scope, sourceKey) {
      const hit = scopes.get(scope)
      if (hit === undefined) return false
      return hit.localOcr.has(this.resolveSource(scope, sourceKey))
    },

    degradedEvidenceTokens(scope) {
      const hit = scopes.get(scope)
      if (hit === undefined || !hit.allFailed || hit.localOcr.size === 0) return []
      const tokens = [...hit.localOcr.keys()]
      for (const [artifactToken, rootSource] of hit.derivedArtifacts) {
        if (hit.localOcr.has(rootSource)) tokens.push(artifactToken)
      }
      return tokens
    },

    recordDerivedArtifact(scope, sourceKey, artifactToken) {
      const token = typeof artifactToken === 'string' ? artifactToken : ''
      if (token === '') return
      const hit = entry(scope)
      const rootSource = this.resolveSource(scope, sourceKey)
      if (rootSource === '') return
      hit.derivedArtifacts.delete(token)
      hit.derivedArtifacts.set(token, rootSource)
      while (hit.derivedArtifacts.size > evidenceLimit) hit.derivedArtifacts.delete(hit.derivedArtifacts.keys().next().value)
    },

    degradedRefinementCount(scope, sourceKey) {
      const hit = scopes.get(scope)
      if (hit === undefined) return 0
      return hit.degradedRefinements.get(this.resolveSource(scope, sourceKey)) ?? 0
    },

    recordDegradedRefinement(scope, sourceKey) {
      const hit = entry(scope)
      const key = this.resolveSource(scope, sourceKey)
      if (key === '') return 0
      const count = (hit.degradedRefinements.get(key) ?? 0) + 1
      hit.degradedRefinements.delete(key)
      hit.degradedRefinements.set(key, count)
      while (hit.degradedRefinements.size > evidenceLimit) hit.degradedRefinements.delete(hit.degradedRefinements.keys().next().value)
      return count
    },
