...
const LOCAL_CAPABILITY_READS = new Map([
...
function redactCapabilityToken(body) {
...
/** Intercept one JSON end() so a local mutation capability is never issued remotely. */
function runWithRemoteCapabilityRedaction(handler, req, res) {
  ...
  res.end = function redactedEnd(chunk, ...args) {
    ...
    try {
      if (typeof chunk === 'string' || Buffer.isBuffer(chunk) || chunk instanceof Uint8Array) {
        ...
        next = JSON.stringify(redactCapabilityToken(parsed))
  ...
  // Capture the concrete function after assignment for identity-safe cleanup.
...
function guardedRoute(route) {
  ...
  const secretReadMethods = LOCAL_CAPABILITY_READS.get(route.path)
  if ((!mutationMethods || mutationMethods.size === 0) && (!secretReadMethods || secretReadMethods.size === 0)) {
    return route
  }
  ...
  return {
    ...
    handler(req, res) {
      ...
      if (secretReadMethods?.has(method) && !localUi) {
        return runWithRemoteCapabilityRedaction(originalHandler, req, res)
      }
 ...
 * not issue their bearer capability tokens to remote read-only clients.
 */
