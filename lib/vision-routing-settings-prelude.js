...
export const VISION_ROUTING_SETTINGS_PRELUDE = String.raw`(function(){
  ...
  function buildPreview(state, settings) {
    ...
    count.textContent = text('实测 ' + measured, measured + ' measured');
    ...
    note.textContent = text(
      settings.mode === 'auto'
        ? '使用同一模型身份和当前Benchmark规则下已有的直接实测；测评时间只作记录，不会因过了几天自动失效。未测或不可直接比较时保持你的设置顺序。Auto执行仅影响Router自有视觉工具，异常或撤权时回退到设置顺序。'
        : '使用同一模型身份和当前Benchmark规则下已有的直接实测；测评时间只作记录，不会因过了几天自动失效。当前为固定顺序；这里展示开启Auto后的执行顺序。',
      settings.mode === 'auto'
        ? 'Uses existing direct measurements for the same model identity and current Benchmark contract. Measurement age is informational and does not expire evidence. Unmeasured or incomparable routes keep your configured order. Auto execution affects router-owned visual tools only and falls back to configured order on errors or authority revocation.'
