...
export const VISION_ROUTING_SETTINGS_PRELUDE = String.raw`(function(){
  ...
  function buildPreview(state, settings) {
    ...
    title.textContent = settings.mode === 'auto'
      ? text('Auto执行顺序', 'Auto execution order')
      : text('Auto选择预览（若开启）', 'Auto selection preview (if enabled)');
    var count = document.createElement('span');
    count.className = 'vr-routing-badge';
    var measured = state.preview && Array.isArray(state.preview.measuredBackends)
      ? state.preview.measuredBackends.length : 0;
    var total = Array.isArray(settings.providers) ? settings.providers.length : 0;
    count.textContent = total > 0
      ? text('实测 ' + measured + ' / ' + total, measured + ' / ' + total + ' measured')
      : text('实测 ' + measured, measured + ' measured');
    ...
    note.textContent = text(
      settings.mode === 'auto'
        ? 'Auto只使用同一模型身份和当前Benchmark规则下已有的直接实测；未测或不可直接比较时保持你的设置顺序。测评时间只作记录，不会因过了几天自动失效。Auto执行仅影响Router自有视觉工具，异常或撤权时回退到设置顺序。'
        : '使用同一模型身份和当前Benchmark规则下已有的直接实测；测评时间只作记录，不会因过了几天自动失效。当前为固定顺序；这里展示开启Auto后的执行顺序。',
      settings.mode === 'auto'
        ? 'Auto uses only existing direct measurements for the same model identity and current Benchmark contract. Unmeasured or incomparable routes keep your configured order. Measurement age is informational and does not expire evidence. Auto execution affects router-owned visual tools only and falls back to configured order on errors or authority revocation.'
        : 'Uses existing direct measurements for the same model identity and current Benchmark contract. Measurement age is informational and does not expire evidence. Fixed-order mode is active; this shows the execution order Auto would use.'
    );
