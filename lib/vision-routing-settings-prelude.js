...
export const VISION_ROUTING_SETTINGS_PRELUDE = String.raw`(function(){
  ...
  function buildPreview(state, settings) {
    ...
    note.textContent = text(
      '使用同一模型身份和当前Benchmark规则下已有的直接实测；测评时间只作记录，不会因过了几天自动失效。未测或不可直接比较时保持你的设置顺序。当前2.0 Draft只显示预览，不会改变实际识图执行顺序。',
      'Uses existing direct measurements for the same model identity and current Benchmark contract. Measurement age is informational and does not expire evidence. Unmeasured or incomparable routes keep your configured order. This 2.0 Draft is preview-only and does not change actual vision execution.'
    );
