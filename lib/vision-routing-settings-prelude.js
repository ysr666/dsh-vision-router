...
export const VISION_ROUTING_SETTINGS_PRELUDE = String.raw`(function(){
  ...
  function previewSettingsKey(settings) {
    return compactKey([settings.mode, settings.preference, settings.providers, settings.revision]);
  }
  ...
  function render(state) {
    ...
    sub.textContent = text(
      '设置顺序始终是基础；自动选择只在新鲜实测结果足够明确时建议调整首选模型。',
      'Configured order remains the baseline. Auto select changes the suggested first model only when fresh measurements are clearly comparable.'
    );
