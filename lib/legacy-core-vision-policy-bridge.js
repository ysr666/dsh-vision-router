...
function configView(config, state) {
  ...
  return new Proxy(config, {
    get(target, property) {
      const projected = projectedConfig(target, state)
      const value = Reflect.get(projected, property, projected)
      return typeof value === 'function' ? value.bind(projected) : value
    },
    has(target, property) {
      return property in projectedConfig(target, state)
    },
    ownKeys(target) {
      return Reflect.ownKeys(projectedConfig(target, state))
    },
...
function rewriteTextOnlyDecision(payload, decision, rewriteHistoryImages) {
  const policy = currentSessionVisionPolicy()
  if (policy?.rewriteCurrentImages !== true || typeof rewriteHistoryImages !== 'function') {
    return decision
  }
  ...
  const rewritten = rewriteHistoryImages(source)
 ...
 * decision through index.js's exported rewriteHistoryImages implementation.
 * No marker/rewrite algorithm is duplicated here.
 */
