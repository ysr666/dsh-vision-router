...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const zh = {
      ...
      hintInstantDescribe: '开启后（需同时开启至少一个本地视觉后端），图片轮第一轮直接用本地模型识别图片块——模型第一轮即看懂，不必先调用识图工具；Ollama 失败会继续尝试 LM Studio。一次贴多张图时并发识别（上限 3，本地推理受显存限制），单张失败自动跳过、不影响其余；整体失败则回退为工具提示标记。会话日志仍保留原图。',
      ...
      toggleDesktopScreenshot: '允许桌面截屏',
      toggleStealth: '隐身模式',
      hintRouting:
      ...
      hintDesktopScreenshot: '隐私敏感功能：允许模型调用 vision_screenshot 截取当前桌面。默认关闭；仅在你确实需要让 Agent 查看桌面时显式开启。',
      hintStealth:
    ...
    const en = {
      ...
      toggleDesktopScreenshot: 'Allow desktop screenshots',
      ...
      hintDesktopScreenshot: 'Privacy-sensitive: allows the model to call vision_screenshot and capture the current desktop. Off by default; enable only when you want the agent to inspect your desktop.',
      hintStealth:
    ...
    const ADVANCED_TOGGLE_KEYS = ['reverseRouting', 'rewriteImages', 'downscale', 'cache', 'freeFallback', 'desktopScreenshot']
    ...
    const CSS =
      ...
      '.vr-local-options{display:flex;align-items:center;gap:14px;flex-wrap:wrap;min-width:0}' +
      '.vr-local-style{display:inline-flex;align-items:center;gap:6px;flex:none;white-space:nowrap}' +
    ...
    function VisionRouterCard(props) {
      ...
      // scroll-adjacent re-renders) walks and copies every group × model in
      // large catalogs. Stable model-array identities are also required for
      // the option VNode cache below to hit instead of growing per keystroke.
      ...
      const visionGroupIndex = useMemo(
        () =>
          new Map(
            visionGroups.map((group) => [
              group.id,
              {
                group,
                modelIds: new Set(
                  (Array.isArray(group.models) ? group.models : []).map((model) => model.id),
                ),
              },
            ]),
          ),
      ...
      const visionModelsFor = (providerId) => {
        const hit = visionGroupIndex.get(providerId)
        return hit && Array.isArray(hit.group.models) ? hit.group.models : []
      }
      const visionProviderVisible = (providerId) =>
        typeof providerId === 'string' && visionGroupIndex.has(providerId)
      const visionModelVisible = (providerId, modelId) =>
        typeof modelId === 'string' && visionGroupIndex.get(providerId)?.modelIds.has(modelId) === true
      React.useEffect(() => {
      ...
      // WeakMap avoids retaining obsolete catalog arrays and their VNodes after
      // a catalog/capability refresh while preserving identity-based cache hits.
      const modelOptionCache = React.useRef(new WeakMap())
      ...
      const describeStyleEditor = () => {
        ...
        return h('div', { className: 'vr-field' },
          ...
          h('div', { className: 'vr-local-options' },
            ['plain', 'structured'].map((style) =>
              h('label', { className: 'vr-label vr-local-style', key: style },
                h('input', {
