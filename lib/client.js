...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const zh = {
      ...
      hintAutoWrapProviders: '默认开启：自动发现「设置 → 模型」里当前已启用的路由，为其中模型生成额外的「+ 自动识图」入口；原模型完全不变，原生多模态模型也会保留原入口并额外提供 vision-router 入口。关闭后仅使用下方手动「额外识图包装」。改动即时生效。',
      hintRewriteImages:
      ...
      wrapHint: '默认会自动发现并包装「设置 → 模型」里已启用的模型；这里用于手动限定某个 provider 的模型范围，或在关闭「自动包装已有模型」后手动指定。每行选一个 provider + 一个模型；模型留空 = 该路由全部模型。原模型始终保留不变。改动即时生效，无需重启。',
    ...
    const en = {
      ...
      hintAutoWrapProviders: 'On by default: discovers routes currently enabled in Settings → Models and creates an additional + auto-vision entry for their models. The original route is never changed; native multimodal models keep their native entry and also get an optional vision-router entry. Turn this off to use only the manual Extra vision wrappers below. Applies immediately.',
      hintRewriteImages:
