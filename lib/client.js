...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function VisionRouterCard(props) {
      ...
      const declaresImage = (model) =>
        model && Array.isArray(model.input) && model.input.includes('image')
      // The wrappers editor only offers TEXT routes to wrap: groups whose
      // models are all image-capable already (twins, the wrapper, vision-http,
      // the vision chain) are pointless wrap targets. deepseek-official stays
      // because it is the pre-filled default row (served by the built-in
      // wrapper) and the stock model users expect to see.
      const wrapGroupOptions = catalog.groups
        .filter((group) => {
          if (group.id === 'deepseek-official') return true
          const models = Array.isArray(group.models) ? group.models : []
          if (models.length === 0) return false
          return !models.every((model) => declaresImage(model))
        })
        .map((group) =>
          h('option', { value: group.id, key: group.id },
            (group.name && group.name !== group.id ? group.name + ' (' + group.id + ')' : group.id)),
        )
      const wrapModelsOf = (providerId) =>
        modelsOf(providerId).filter((model) => !declaresImage(model))
      const chainEditor = () => {
          ...
          rows.map((row, index) =>
            h('div', { className: 'vr-chain-row', key: index },
              ...
              h('select', {
                ...
                wrapModelsOf(row.provider).map((model) =>
                  h('option', { value: model.id, key: model.id },
                    (model.name && model.name !== model.id ? model.name + ' (' + model.id + ')' : model.id) +
                    (row.provider === 'vision-http' ? t('freeTag') : '')),
                ),
