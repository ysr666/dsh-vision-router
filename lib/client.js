...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const zh = {
      ...
      toggleTool: '识图工具',
      toggleAutoWrapProviders: '自动包装已有文本模型',
      ...
      hintAutoWrapProviders: '默认开启：自动发现「设置 → 模型」里当前已启用的文本路由，为尚未声明图片能力的模型生成「+ 自动识图」入口；原生已支持图片的模型不会重复包装。关闭后仅使用下方手动「额外识图包装」。改动即时生效。',
      hintRewriteImages:
      ...
      wrapHint: '默认会自动发现并包装「设置 → 模型」里已启用、尚未声明图片能力的文本模型；这里用于手动限定某个 provider 的模型范围，或在关闭「自动包装已有文本模型」后手动指定。每行选一个 provider + 一个模型；模型留空 = 该路由全部文本模型。原生已支持图片的模型不会重复包装。改动即时生效，无需重启。',
    ...
    const en = {
      ...
      toggleAutoWrapProviders: 'Auto-wrap configured text models',
      ...
      hintAutoWrapProviders: 'On by default: discovers the text routes currently enabled in Settings → Models and creates a + auto vision entry for models that do not already declare image input. Native vision models are not duplicated. Turn this off to use only the manual Extra vision wrappers below. Applies immediately.',
      hintRewriteImages:
      ...
      wrapHint: 'By default the plugin automatically discovers enabled text models from Settings → Models and wraps those that do not already declare image input. Use this section to narrow a provider to specific models, or to choose wrappers manually after turning off Auto-wrap configured text models. Leave the model empty to wrap all text-only models on that route. Native vision models are never duplicated. Changes apply immediately.',
