...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const zh = {
      ...
      textHintWrappedProviders: '给 opencode 等任意第三方/自定义文本路由注册「自动识图」孪生条目，选择器里就能选中它来发图。每行 `provider` 包装全部模型，或 `provider/model1,model2` 只包装指定模型；重启 dsh 生效。',
      wrapHint: '给 opencode 等任意第三方/自定义文本路由注册「自动识图」孪生条目，选择器里就能选中它来发图。每行选一个 provider + 一个模型；模型留空（全部模型）= 包装该路由全部模型，同一 provider 要包装多个模型就添加多行。重启 dsh 生效。',
    ...
    const en = {
      ...
      textHintWrappedProviders: 'Registers an auto-vision twin for any third-party/custom text route (e.g. opencode) so you can pick it in the model selector and send images. One `provider` per line wraps all of its models; `provider/model1,model2` wraps specific models. Takes effect after a dsh restart.',
      wrapHint: 'Registers an auto-vision twin for any third-party/custom text route (e.g. opencode) so you can pick it in the model selector and send images. Each row = one provider + one model; leave the model empty (all models) to wrap every model of that route — add more rows to wrap several models of the same provider. Takes effect after a dsh restart.',
