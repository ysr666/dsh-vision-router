      ...
      const loadCatalog = () => {
        ...
        try {
          ...
          if (modelsFn === undefined) {
            setCatalog({ status: 'error', groups: [], error: '模型目录不可用，退回手动输入。' })
            return
          }
          modelsFn({}).then(
            (value) => {
      ...
      return h('li', { className: 'vr-card' + (open ? ' vr-card-open' : '') },
        h('button', {
          ...
          h('span', { className: 'vr-headText' },
            ...
            h('span', { className: 'vr-desc' }, '默认即用内置免费视觉模型；已配置的模型可直接选择，细节收在高级设置里'),
          ),
        ...
        open
          ? h('div', { className: 'vr-body' },
              ...
              catalog.status === 'error'
                ...
                : null,
              h('button', {
                type: 'button', className: 'vr-header', 'aria-expanded': showAdvanced,
                style: { border: 'none', padding: '6px 0', width: '100%', textAlign: 'left' },
