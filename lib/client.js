...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const zh = {
      ...
      textHintWrappedProviders: '给常用的第三方文本模型路由（如 opencode-go）注册「自动识图」孪生条目，选择器里就能选中它来发图。每行 `provider` 包装全部模型，或 `provider/model1,model2` 只包装指定模型；重启 dsh 生效。',
      wrapHint: '先选 provider，再点选要包装的模型；不勾选任何模型 = 包装该路由的全部模型。模型很多的路由只列出前 20 个。重启 dsh 生效。',
      wrapAllModels: '全部模型',
      wrapAllModelsTitle: '点此清空勾选 = 包装全部模型',
      addWrapper: '+ 添加包装',
      wrapPickProviderFirst: '先选择 provider，再勾选模型',
      wrapMoreModels: '（其余模型未列出；留空 = 包装全部）',
      textProviders: '视觉模型链',
    ...
    const en = {
      ...
      textHintWrappedProviders: 'Registers an auto-vision twin entry for third-party text routes you use (e.g. opencode-go), so you can pick it in the model selector and send images. One `provider` per line wraps all of its models; `provider/model1,model2` wraps specific models. Takes effect after a dsh restart.',
      wrapHint: 'Pick a provider first, then tick the models to wrap; ticking none wraps every model of that route. Routes with many models list only the first 20. Takes effect after a dsh restart.',
      wrapAllModels: 'All models',
      wrapAllModelsTitle: 'Click to clear the ticks = wrap all models',
      addWrapper: '+ Add wrapper',
      wrapPickProviderFirst: 'Pick a provider first, then tick models',
      wrapMoreModels: ' (more models not listed; empty = wrap all)',
    ...
    const CSS =
      ...
      '.vr-chips{display:flex;flex-wrap:wrap;gap:6px;margin:2px 0 6px}' +
      '.vr-chip{appearance:none;font:inherit;cursor:pointer;border:1px solid var(--dsw-alias-border-l2);background:0 0;color:var(--dsw-alias-label-secondary);border-radius:999px;padding:2px 10px;font-size:12px;line-height:18px}' +
      '.vr-chip:hover:not(:disabled){color:var(--dsw-alias-label-primary);border-color:var(--dsw-alias-label-dimmed)}' +
      '.vr-chip-on{background:var(--dsw-alias-brand-primary);border-color:var(--dsw-alias-brand-primary);color:var(--dsw-alias-bg-layer-3)}' +
      '.vr-chip-on:hover:not(:disabled){color:var(--dsw-alias-bg-layer-3);border-color:var(--dsw-alias-brand-primary)}' +
      '.vr-chip:disabled{opacity:.4;cursor:default}' +
      '.vr-wrap-row{border-top:1px solid var(--dsw-alias-border-l2);padding:8px 0 2px}' +
    ...
    function VisionRouterCard(props) {
          ...
          if (catalogReady) {
            ...
            for (const row of rows) {
              ...
              const models = Array.isArray(row.models)
                ? row.models.filter((model) => typeof model === 'string' && model.trim() !== '').map((model) => model.trim())
                : []
              if (provider === '') {
                // the empty template row is fine only when no models were picked
                if (models.length > 0) return undefined
                continue
              }
              cleaned.push({ provider, models })
            }
      ...
      const wrappersEditor = () => {
        ...
        const rows = Array.isArray(value) && value.length > 0 ? value : [{ provider: '', models: [] }]
        ...
        const removeWrap = (index) => {
          const list = rows.filter((_row, i) => i !== index)
          setDraft('wrappedProviders', list.length > 0 ? list : [{ provider: '', models: [] }])
        }
        const toggleModel = (row, modelId) => {
          const models = Array.isArray(row.models) ? row.models.slice() : []
          const at = models.indexOf(modelId)
          if (at === -1) models.push(modelId)
          else models.splice(at, 1)
          return { ...row, models }
        }
        const MAX_CHIPS = 20
        return h('div', { className: 'vr-field' },
          ...
          rows.map((row, index) => {
            ...
            const allModels = row.provider && models.length === 0
            const groupModels = modelsOf(row.provider)
            const chips = groupModels.slice(0, MAX_CHIPS)
            const hiddenCount = groupModels.length - chips.length
            return h('div', { className: 'vr-wrap-row', key: index },
              h('div', { className: 'vr-chain-row' },
                h('select', {
                  className: 'vr-input vr-select', value: row.provider ?? '', disabled: !writable,
                  onChange: (event) => updateWrap(index, { provider: event.target.value, models: [] }),
                },
                  h('option', { value: '' }, t('selectProvider')),
                  groupOptions,
                ),
              ...
              row.provider
                ? h('div', { className: 'vr-chips' },
                    h('button', {
                      type: 'button',
                      className: 'vr-chip' + (allModels ? ' vr-chip-on' : ''),
                      disabled: !writable,
                      title: t('wrapAllModelsTitle'),
                      onClick: () => updateWrap(index, { ...row, models: [] }),
                    }, t('wrapAllModels')),
                    chips.map((model) => {
                      const on = models.includes(model.id)
                      return h('button', {
                        type: 'button', key: model.id, title: model.id,
                        className: 'vr-chip' + (on ? ' vr-chip-on' : ''),
                        disabled: !writable,
                        onClick: () => updateWrap(index, toggleModel(row, model.id)),
                      },
                        model.id + (row.provider === 'vision-http' ? t('freeTag') : ''))
                    }),
                    hiddenCount > 0
                      ? h('span', { className: 'vr-hint' }, t('wrapMoreModels'))
                      : null,
                  )
                : h('p', { className: 'vr-hint' }, t('wrapPickProviderFirst')),
            )
          }),
          h('button', {
            type: 'button', className: 'vr-btn', disabled: !writable,
            onClick: () => setDraft('wrappedProviders', [...rows, { provider: '', models: [] }]),
          }, t('addWrapper')),
