...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function VisionRouterCard(props) {
      ...
      useEffect(() => {
        let live = true
        // Defensive to the point of paranoia: `connection` may be a sandbox
        // proxy whose property traps THROW on unknown members, and an effect
        // error unmounts the whole settings tab. Any failure just falls back
        // to the free-text model inputs.
        let modelsFn
        try {
          const api = connection && connection.api
          modelsFn = api && api.llm && typeof api.llm.models === 'function' ? api.llm.models : undefined
        } catch {
          modelsFn = undefined
        }
        ...
        try {
          ...
          if (live) setCatalog({ status: 'error', groups: [], error: '模型目录获取失败，退回手动输入。' })
        }
        return () => {
          live = false
        }
    ...
    function apply(ctx) {
      ...
      // Lazy, never a hard inject: the third-party bundle ctx may not expose the
      // `connection` service, and a parked inject would keep the whole card from
      // registering. Without it the card still works — model fields fall back to
      // free-text inputs.
      const getConnection = () => {
        try {
            ...
            yield ctx.slots.register(
              {
                ...
                order: 30,
                label: '视觉路由（自动识图）',
                inject: () => ({ scope, connection: getConnection() }),
              },
              VisionRouterCard,
            )
          }),
