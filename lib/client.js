...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const zh = {
      ...
      hintInstantDescribe: '开启后（需同时开启本地 Ollama），图片轮第一轮直接用本地模型识别图片块——模型第一轮即看懂，不必先调用识图工具；本地识别失败自动回退为工具提示标记。会话日志仍保留原图。',
    ...
    const en = {
      ...
      hintInstantDescribe: 'When on (and local Ollama is enabled), image blocks are recognized by the local model on the first model step — the model understands the image immediately without calling a vision tool first. Failures fall back to the tool-hint marker. The session log keeps the original image.',
        ...
        return h('div', { className: 'vr-field' },
          ...
          h('div', { className: 'vr-local-row vr-local-row-pair' },
            ...
            h('input', {
              ...
              disabled: !writable, placeholder: '0.5（建议）', type: 'number', step: '0.1', min: '0', max: '2',
            ...
            h('input', {
              ...
              disabled: !writable, placeholder: '0.8（建议）', type: 'number', step: '0.05', min: '0', max: '1',
              onChange: (event) => update({
      ...
      return h('li', { className: 'vr-card' + (open ? ' vr-card-open' : '') },
        ...
        open
          ? h('div', { className: 'vr-body' },
              ...
              stealthNotice(),
              // dsh-vision 并入：本地视觉分组（即时翻译 + 本地 Ollama + 风格）
              h('div', { className: 'vr-group' },
                h('p', { className: 'vr-group-title' }, t('groupLocalOllama')),
