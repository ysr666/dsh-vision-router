 ...
 * rejected settings.mutate result and publishes only the recovery read. This
 * path calls the SettingsProvider directly and verifies the raw user layer
 ...
 * operation to the loopback-only Host endpoint, then overlays the endpoint's
 * verified result until DSH's own SettingsScope refresh catches up. Remote
 * scopes and every other field keep the stock DSH path.
 */
...
export function installLocalRemoteSettingsPermissionBridge(ctx, logger) {
  // A real Cordis Host Context always has get(). Tiny unit-test facades for the
  // trusted-host remote bridge intentionally do not, and should not be forced
  // to grow a WebServer just because this independent local capability exists.
