...
function syncAgentToolRestriction(agent, enabled, restrictions, owned) {
  ...
  try {
    ...
    // Fail closed at execution even when an older/foreign Host rejects the
    // scoped visibility mask. Keep the reason observable because a rejected
    // restriction otherwise looks exactly like a successful OFF transition
    // until the model-visible schema leaks on a real request.
    try {
      agent?.ctx?.logger?.warn?.(
        'vision-router: failed to restrict Session vision tools while Vision mode is off: %s (deny=%s)',
