 ...
 * and projects visibility/execution authority per Agent.
 *
...
export function installSessionVisionModeBoundary(ctx, config = {}) {
  ...
  const clearAgent = (agent) => {
    ...
    if (isObject(agent)) assemblySnapshots.delete(agent)
  }
  ...
  const captureForAssembly = (agent) => {
    ...
    clearSessionVisionModeAuthorityForAgent(agent)
    const authority = resolveForAgent(agent)
    assemblySnapshots.set(agent, authority)
  ...
  // Code Mode's tools:sdk are generated. The returned variable is unused and
  ...
  try {
    ctx.on?.('system-prompt/assemble', async (assembly, context, next) => {
      ...
      let authority = isObject(agent) ? assemblySnapshots.get(agent) : undefined
      if (!authority && isObject(agent)) authority = captureForAssembly(agent)
      const transformed = typeof next === 'function' ? await next() : assembly
      return projectAssemblyTools(transformed, authority?.enabled === true, ownedVisionTools)
    })
  } catch {
