...
import {
  ...
  rememberSessionVisionModeAuthorityForAgent,
  resolveSessionVisionModeAuthority,
  runWithSessionVisionModeAuthority,
  sessionVisionModeAuthorityForAgent,
...
export function installSessionVisionModeBoundary(ctx, config = {}) {
  ...
  const captureForAssembly = (agent) => {
    ...
    const authority = resolveForAgent(agent)
    rememberSessionVisionModeAuthorityForAgent(authority, agent)
    syncAgentToolRestriction(ctx, agent, authority.enabled === true, restrictions)
    return authority
  }
  ...
  // PTC's generated tools:sdk section on every assembled step. The returned
  // variable is intentionally unused and therefore never reaches the model.
  try {
    ...
    if (typeof systemPrompt?.variable === 'function') {
      systemPrompt.variable(MODE_SYNC_VARIABLE, (context) => {
        captureForAssembly(context?.agent)
        return ''
      })
    }
  } catch {
      ...
      if (property === 'on') {
        ...
        return (event, handler, ...rest) => {
          ...
          return on.call(target, event, async function sessionVisionModeAwarePreStep(payload, next) {
            ...
            // Prompt assembly is the exact DSH selection-snapshot boundary. If
            // it captured authority already, preserve that decision for this
            // step even if the picker changes concurrently before pre-step.
            const authority = sessionVisionModeAuthorityForAgent(agent) ?? resolveSessionVisionModeAuthority(
              target,
              agent,
