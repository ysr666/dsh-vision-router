...
export const CAPABILITY_PROFILE_CACHE_VERSION = 2
...
export function createCapabilityProfileStore(options = {}) {
  ...
  return {
    ...
    async put(record) {
      ...
      if (existing && Number(existing.fixtureCount) > Number(clean.fixtureCount)) {
        const scoreKeys = Object.keys(clean.scores)
        const groundingRepair =
          Number(clean.fixtureCount) === 1 &&
          Number(clean.failureCount) === 0 &&
          scoreKeys.length === 1 &&
          scoreKeys[0] === 'grounding' &&
          clean.groundingDiagnostic
        if (!groundingRepair) return existing

        const groundingLatency = Number(clean.medianLatencyMs?.grounding)
        const merged = sanitizeCapabilityProfileRecord({
          ...existing,
          scores: {
            ...existing.scores,
            grounding: clean.scores.grounding,
          },
          medianLatencyMs: {
            ...existing.medianLatencyMs,
            ...(Number.isFinite(groundingLatency) && groundingLatency >= 0 ? { grounding: groundingLatency } : {}),
          },
          groundingDiagnostic: clean.groundingDiagnostic,
        })
