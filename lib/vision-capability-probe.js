...
export const DEFAULT_CAPABILITY_PROFILE_MAX_AGE_MS = 90 * 24 * 60 * 60 * 1000
...
export async function runExactCapabilityBenchmark({
  ...
  for (const fixture of fixtures) {
    ...
    try {
      ...
      if (error?.name === 'AbortError' || error?.code === 'CAPABILITY_BENCHMARK_BACKEND_MISMATCH') throw error
      const finished = Number(now())
      const latencyMs = Number.isFinite(started) && Number.isFinite(finished) ? Math.max(0, finished - started) : undefined
      failureCount += 1
      results.push({
