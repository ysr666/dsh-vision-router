...
export const CAPABILITY_PROFILE_CACHE_VERSION = 3
export const DEFAULT_CAPABILITY_PROFILE_MAX_ENTRIES = 128
// Measurement age is informational. The default product contract does not
// expire capability evidence just because time passed; identity/suite changes
// invalidate evidence instead. Finite limits remain available only when an
// explicit caller deliberately supplies one for cache-policy/testing purposes.
export const DEFAULT_CAPABILITY_PROFILE_MAX_AGE_MS = Number.MAX_SAFE_INTEGER
export const DEFAULT_CAPABILITY_PROFILE_FRESH_MS = Number.MAX_SAFE_INTEGER
...
export function capabilityProfileAxisFreshness(
  record,
  axis,
  now = Date.now(),
  freshMs = DEFAULT_CAPABILITY_PROFILE_FRESH_MS,
  retainMs = DEFAULT_CAPABILITY_PROFILE_MAX_AGE_MS,
) {
...
export function sanitizeCapabilityProfileRecord(record) {
  ...
  const medianLatencyMs = Object.fromEntries(
    Object.entries(cleanLatencyMap(record.medianLatencyMs)).filter(([axis]) => Object.prototype.hasOwnProperty.call(scores, axis)),
  )
  ...
  const groundingDiagnostic = Object.prototype.hasOwnProperty.call(scores, 'grounding')
    ...
    : undefined
  const latencyMs = aggregateLatency(medianLatencyMs)
...
function mergeCapabilityProfiles(existing, incoming) {
  ...
  const medianLatencyMs = { ...existing.medianLatencyMs }
  ...
  for (const [axis, score] of Object.entries(incoming.scores ?? {})) {
    ...
    if (Number.isFinite(Number(incoming.medianLatencyMs?.[axis]))) medianLatencyMs[axis] = Number(incoming.medianLatencyMs[axis])
    else delete medianLatencyMs[axis]
...
function pruneExpiredAxes(record, cutoff) {
  if (!record) return undefined
  const scores = {}
  const measuredAtByAxis = {}
  const medianLatencyMs = {}
  const fixtureCountByAxis = {}
  for (const [axis, score] of Object.entries(record.scores ?? {})) {
    const measuredAt = capabilityProfileAxisMeasuredAt(record, axis)
    if (!Number.isFinite(measuredAt) || measuredAt < cutoff) continue
    scores[axis] = score
    measuredAtByAxis[axis] = measuredAt
    if (Number.isFinite(Number(record.medianLatencyMs?.[axis]))) medianLatencyMs[axis] = Number(record.medianLatencyMs[axis])
    fixtureCountByAxis[axis] = Math.max(1, Math.floor(Number(record.fixtureCountByAxis?.[axis]) || 1))
  }
  return sanitizeCapabilityProfileRecord({
    ...record,
    scores,
    ...
    medianLatencyMs,
    fixtureCountByAxis,
    groundingDiagnostic: Object.prototype.hasOwnProperty.call(scores, 'grounding') ? record.groundingDiagnostic : undefined,
  })
}
...
async function loadCache(file, fsOps, { now, maxAgeMs }) {
  try {
    ...
    if (!body || ![LEGACY_CACHE_VERSION, CAPABILITY_PROFILE_CACHE_VERSION].includes(body.version) || !Array.isArray(body.profiles)) return new Map()
    const cutoff = Number(now()) - maxAgeMs
    return new Map(
      body.profiles
        .map(sanitizeCapabilityProfileRecord)
        .map((record) => pruneExpiredAxes(record, cutoff))
        .filter(Boolean)
