...
export function installLegacyGlobalProxyBoundary(ctx, config = {}, options = {}) {
  ...
  const dispatcherFor = (url) => {
    ...
    const agentPromise = Promise.resolve()
      .then(() => importUndici())
      .then(({ ProxyAgent }) => {
        if (typeof ProxyAgent !== 'function') throw new Error('dsh-vision-router: undici ProxyAgent is unavailable')
  ...
  const scopedFetch = (input, init) => {
    ...
    if (!proxyHostMatchesAny(url.hostname, proxyHostsOf(current))) return originalFetch(input, init)
    const effectiveProxyUrl = effectiveProxyUrlForUndici(proxyUrl)
    return dispatcherFor(effectiveProxyUrl).then((dispatcher) =>
