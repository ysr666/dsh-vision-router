...
import { ProxyDispatcherTracker } from './runtime-reliability.js'
import {
...
export function installLegacyGlobalProxyBoundary(ctx, config = {}, options = {}) {
  ...
  const compatDispatcherFor = (proxyUrl) => {
    ...
    // A live Settings change must not retain the old SOCKS pool indefinitely.
    if (cachedCompatProxyUrl !== undefined && cachedCompatProxyUrl !== proxyUrl) {
      retireCompatDispatcher()
    }
    cachedCompatProxyUrl = proxyUrl
    cachedCompatDispatcherPromise = Promise.resolve(importUndici()).then(({ ProxyAgent }) => {
      if (typeof ProxyAgent !== 'function') {
        throw new Error('vision-router: undici ProxyAgent is unavailable')
      }
      const dispatcher = new ProxyAgent(proxyUrl)
      proxyDispatchers.observe(dispatcher)
      return dispatcher
    }).catch((error) => {
  ...
  const gatedFetch = (...args) => {
    ...
    if (!legacyGlobalProxyRequired(current)) {
      retireCompatDispatcher()
      return originalFetch(...args)
    }

    const proxyUrl = currentProxyUrl(current)
    if (!needsSocks5hProxyCompat(proxyUrl)) {
      retireCompatDispatcher()
