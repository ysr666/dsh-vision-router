...
// for an EXPLICIT Vision Router proxy override used by Host-owned/raw-fetch visual
// providers. With no plugin proxy configured, DSH/Host remains the sole network
// authority and this legacy seam is bypassed entirely.

import { visionProxyOverrideConfigured } from './proxy-routing.js'
...
export const LEGACY_GLOBAL_PROXY_REMOVAL_CONDITION =
  'Remove when explicit Vision Router proxy overrides no longer need process-global mutation for supported Host-owned visual providers, or when that override is retired by product policy.'
...
export function legacyGlobalProxyRequired(config = {}) {
  if (!visionProxyOverrideConfigured(config)) return false
 ...
 * visual chain is Router-owned OR no explicit Vision Router proxy override is
 * configured. Only the combination of an explicit plugin proxy + Host-owned
 * provider can fall through to the legacy patched fetch. No request URL, header
 * or body is rewritten here; this boundary only decides whether the old
