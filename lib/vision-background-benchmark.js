...
async function restorePersistentStops(backoff, candidates, backgroundStopStore) {
...
function candidateBlockedByNonRetryableFailure(backoff, candidate) {
...
async function chooseNextWork({ ctx, config, core, store, now, backoff, excluded, imageVerdictStore, backgroundStopStore }) {
  ...
  await restorePersistentStops(backoff, collected, backgroundStopStore)
  const candidates = []
  for (const candidate of collected) {
    ...
    if (candidateBlockedByNonRetryableFailure(backoff, candidate)) continue
...
export function createBackgroundCapabilityProfiler({
  ...
  const candidateForSelection = async (provider, model) => {
    ...
    const live = activeSettings(ctx, config)
    const candidates = await collectCapabilityShadowCandidates(ctx, live, core, store)
  ...
  const tick = async () => {
    ...
    try {
      ...
      if (yielded) {
        ...
        if (failure.errorClass === 'unsupported-image') await markCandidateUnsupported(work.candidate)
        ...
        if (failure.retryable === false && failure.errorClass !== 'unsupported-image' && typeof backgroundStopStore?.mark === 'function') {
          try {
