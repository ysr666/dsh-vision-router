...
async function chooseNextWork({ ctx, config, core, store, now, backoff, excluded, imageVerdictStore }) {
  const authority = resolveVisionRoutingAuthority(config)
  excluded.clear()
...
export function installBackgroundCapabilityProfiling(ctx, config, core, store, options = {}) {
  const imageVerdictStore = options.imageVerdictStore ?? createImageInputVerdictStore({ logger: options.logger ?? ctx?.logger })
