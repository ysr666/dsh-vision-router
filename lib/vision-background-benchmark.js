...
async function chooseNextWork({ ctx, config, core, store, now, preferredAxis, backoff }) {
  ...
  // Axis-first scheduling rapidly establishes the same basic evidence across
  // every eligible model before deepening one model. This keeps Auto useful
  // when a user enables background profiling for a larger configured pool.
  for (const axis of orderedAxes(preferredAxis)) {
    for (const candidate of candidates) {
...
export function installBackgroundCapabilityProfiling(ctx, config, core, store, options = {}) {
  ...
  try {
    ...
    // Event-driven wakeups are an optimization; the periodic scan remains.
  }
