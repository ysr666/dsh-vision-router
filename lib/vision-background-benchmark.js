...
function workStillAuthorized(work, config, core) {
  ...
  if (product.mode !== 'auto' || mode === 'off') return false
  return candidateCanRunInBackground(work?.candidate, mode, core)
}
...
async function chooseNextWork({ ctx, config, core, store, now, preferredAxis, backoff }) {
  ...
  for (const candidate of candidates) {
    if (!candidateCanRunInBackground(candidate, mode, core)) continue
