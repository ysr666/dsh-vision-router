...
async function candidateCanRunUnattended(ctx, candidate, mode, core) {
  if (!candidateCanRunInBackground(candidate, mode, core)) return false
  ...
  return !(await hostExplicitlyTextOnly(ctx, candidate))
}
...
async function chooseNextWork({ ctx, config, core, store, now, backoff }) {
  ...
  for (const candidate of collected) {
    if (await candidateCanRunUnattended(ctx, candidate, authority.backgroundMeasurement, core)) {
