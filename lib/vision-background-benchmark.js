...
const NON_RETRYABLE_BACKGROUND_FAILURES = new Set([
  'auth',
  'unsupported-image',
  'protocol',
  'infrastructure',
...
async function hostExplicitlyTextOnly(ctx, candidate) {
  ...
  try {
    ...
    // Missing metadata stays advisory. Background measurement is blocked only
    // when the Host positively declares a text-only modality set.
    return false
  }
}

async function unattendedEligibility(ctx, candidate, mode, core) {
  if (!candidateCanRunInBackground(candidate, mode, core)) return { eligible: false }
  // Background measurement is standing automation authority, not permission to
  // contradict a positive Host text-only declaration. Users can still use the
  // explicit Test Vision / Force Verify paths to challenge that advisory.
  if (await hostExplicitlyTextOnly(ctx, candidate)) {
    return { eligible: false, reason: 'host-text-only' }
  }
...
function classifyUnavailable(value) {
  ...
  if (code.includes('MODEL_NOT_FOUND') || code.includes('NOT_FOUND') || code.includes('UNAVAILABLE')) return true
  return /model[^\n]{0,80}(?:not found|does not exist|unavailable|not available|unknown model)|no such model/.test(text)
}
...
export function classifyBackgroundBenchmarkFailure(value) {
  ...
  if (failureClass === 'visual-proof') failureClass = 'unsupported-image'
  ...
  return {
    ...
    retryable: !NON_RETRYABLE_BACKGROUND_FAILURES.has(failureClass),
  }
}
...
function publicBackoffEntries(backoff, at) {
  ...
  for (const entry of backoff.values()) {
    ...
    if (entry.retryable === true && (!Number.isFinite(until) || until <= at)) continue
    out.push({
      key: entry.key,
      axis: entry.axis,
      ...
      ...(entry.retryable === true && Number.isFinite(until) ? { until } : {}),
    })
...
function clearNonRetryableBackoff(backoff) {
  for (const [key, entry] of backoff.entries()) {
    if (entry?.retryable === false) backoff.delete(key)
  }
}

function pruneBackoff(backoff, candidates) {
  const valid = new Set()
  for (const candidate of candidates) {
    for (const axis of orderedAxes()) valid.add(backoffKey(candidate, axis))
  }
  for (const key of backoff.keys()) {
    if (!valid.has(key)) backoff.delete(key)
  }
}

async function chooseNextWork({ ctx, config, core, store, now, backoff, excluded }) {
  const authority = resolveVisionRoutingAuthority(config)
  excluded.clear()
  if (!authority.backgroundMeasurementActive) return undefined
  const collected = await collectCapabilityShadowCandidates(ctx, config, core, store)
  pruneBackoff(backoff, collected)
  const candidates = []
  for (const candidate of collected) {
    const eligibility = await unattendedEligibility(ctx, candidate, authority.backgroundMeasurement, core)
  ...
  for (const axis of orderedAxes()) {
    for (const candidate of candidates) {
      ...
      if (deferred?.retryable === false) continue
      const blockedUntil = Number(deferred?.until || 0)
      if (blockedUntil > now) continue
      if (deferred && blockedUntil <= now) backoff.delete(key)
...
async function assertBackgroundPublishable({ ctx, config, core, store, candidate, axis, signal }) {
  ...
  if (
    ...
    await hostExplicitlyTextOnly(ctx, same)
  ) {
...
export function createBackgroundCapabilityProfiler({
  ...
  const settingsChanged = () => {
    ...
    clearNonRetryableBackoff(backoff)
    excluded.clear()
  ...
  const tick = async () => {
    ...
    let work
    try {
      work = await chooseNextWork({ ctx, config: current, core, store, now: currentNow, backoff, excluded })
    } catch (error) {
    ...
    try {
      ...
      if (yielded) {
        ...
        const until = failure.retryable ? Number(now()) + retryMs : undefined
        backoff.set(key, {
