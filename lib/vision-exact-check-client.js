...
export const VISION_EXACT_CHECK_CLIENT = String.raw`(function(){
  ...
  function dshImageAdmissionHint(selection){
    if(!selection||selection.provider==='vision-http')return '';
    return text(
      '如果模型实测能识图，但 DSH 原生发送图片时仍把它判为纯文本，请在 ~/.dsh/settings.yaml 的 llm-pi-ai 自定义提供方模型项中声明 input: [text, image]；若该提供方手动录入的模型都支持图片，也可在提供方上设置 defaultInput: [text, image]。',
      'If the model passes this image check but DSH still treats it as text-only for native image sends, declare input: [text, image] on that custom llm-pi-ai model in ~/.dsh/settings.yaml. If every hand-entered model on the provider accepts images, you can set provider-level defaultInput: [text, image] instead.'
    );
  }
  ...
  async function run(row,control){
    ...
    try{
      ...
      var admission=dshImageAdmissionHint(selection);
      var admissionLabel=admission?text(' · DSH仍判纯文本？加input:[text,image]',' · DSH still says text-only? add input:[text,image]'):'';
      var success='✓ '+selection.provider+'/'+selection.model+text(' · 图片识别正常',' · image verified')+admissionLabel+suffix;
      setStatus(control,success,admission?success+'\n'+admission:success);notifyChanged();
    }catch(error){
      if(!currentRun(control,token,key))return;
