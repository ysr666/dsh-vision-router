...
import { normalizeProxyUrlForUndici } from './proxy-url-compat.js'
...
export function createVisionProviderTransport({
  ...
  const fetchProvider = async (input, init = {}, context = {}) => {
    ...
    // Canonicalize only after selective-proxy admission so #149's Host fetch
    // boundary remains untouched for disabled/non-matching requests. Using the
    // canonical value as the cache key also avoids rebuilding the same pool
    // when a user changes legacy `socks5h:` spelling to native `socks5:`.
    const dispatcher = await dispatcherFor(normalizeProxyUrlForUndici(proxy))
