...
export const CAPABILITY_BENCHMARK_CLIENT = String.raw`(function(){
  ...
  function coverageKindText(measured){
    var full = measured && (measured.coverageKind === 'full' || coverageOf(measured).length === SCORE_ORDER.length);
    return full ? text('完整测评', 'Full benchmark') : text('部分测评', 'Partial benchmark');
  }
  function axisFreshness(measured, axis){
    var map = measured && measured.freshnessByAxis;
    var value = map && map[axis];
    if (value === 'fresh' || value === 'stale' || value === 'expired') return value;
    if (coverageOf(measured).indexOf(axis) < 0) return 'unknown';
    return measured && (measured.freshness === 'fresh' || measured.freshness === 'stale' || measured.freshness === 'expired') ? measured.freshness : 'unknown';
  }
  ...
  function freshnessCounts(measured){
    var out = { fresh:0, stale:0, expired:0 };
    coverageOf(measured).forEach(function(axis){ var state=axisFreshness(measured,axis); if(Object.prototype.hasOwnProperty.call(out,state))out[state]+=1; });
    return out;
  }
  function freshnessText(measured){
    if (!measured) return '';
    var coverage=coverageOf(measured), counts=freshnessCounts(measured);
    if (!coverage.length) return '';
    if (counts.fresh === coverage.length) return text('全部已测能力可用于自动选择', 'All measured axes are eligible for auto selection');
    if (counts.fresh > 0) return text('部分能力可用于自动选择（'+counts.fresh+'/'+coverage.length+'）', 'Some axes are eligible for auto selection ('+counts.fresh+'/'+coverage.length+')');
    if (counts.stale > 0) return text('已测能力均已陈旧 · 暂不参与自动选择', 'Measured axes are stale · not used for auto selection');
    return text('已测能力已过期', 'Measured axes have expired');
  }
  function axisStateText(measured,axis){
    var state=axisFreshness(measured,axis), age=ageText(axisMeasuredAt(measured,axis));
    if(state==='fresh')return [age,text('可用于Auto','Auto-ready')].filter(Boolean).join(' · ');
    if(state==='stale')return [age,text('已陈旧','stale')].filter(Boolean).join(' · ');
    if(state==='expired')return [age,text('已过期','expired')].filter(Boolean).join(' · ');
    return age || '—';
  }
  function measuredMetaText(measured){
    if (!measured) return '';
    return [coverageKindText(measured) + ' · ' + coverageText(measured), freshnessText(measured)].filter(Boolean).join(' · ');
  }
  function compactMeasuredText(measured){
    var value = coverageKindText(measured) + ' · ' + coverageText(measured);
    var counts=freshnessCounts(measured);
    if(counts.fresh>0)value+=text(' · '+counts.fresh+'项可用于Auto',' · '+counts.fresh+' Auto-ready');
    if(counts.stale>0)value+=text(' · '+counts.stale+'项已陈旧',' · '+counts.stale+' stale');
    return value;
  }
  ...
  function renderControl(row, control, body){
    ...
    else setStatus(control, text('尚未测评 · 自动选择不会推断此模型能力','Not benchmarked · auto selection will not infer capability'));
  ...
  function appendMeasuredDetails(container, measured){
    ...
    var meta=css(document.createElement('div'),{fontSize:'12px',opacity:'0.66',marginTop:'4px'}); meta.textContent=freshnessText(measured); card.appendChild(meta);
    ...
    var headers=[text('能力','Capability'),text('分数','Score'),text('中位延迟','Median'),text('新鲜度','Freshness')];
    ...
    var scores=measured.scores||{}, latencies=measured.medianLatencyMs||{};
  ...
  async function openBenchmarkModal(row,control){
    ...
    if(candidate.measured)appendMeasuredDetails(panel,candidate.measured); else { var empty=css(document.createElement('div'),{marginTop:'16px',opacity:'0.72'}); empty.textContent=candidate.imageCapability==='text-only'?text('DSH当前将此模型标记为仅文本。','DSH currently marks this model as text-only.'):text('尚未测评。自动选择不会根据模型名称推断能力。','Not benchmarked. Auto selection will not infer capability from the model name.');panel.appendChild(empty); }
    ...
    if(candidate.imageCapability==='text-only'&&!candidate.measured){
      ...
      list.appendChild(modalButton(candidate.measured?text('快速重测','Quick retest'):text('快速测评','Quick benchmark'),text('约3次请求 · 覆盖 OCR 和通用','About 3 requests · covers OCR and general'),'quick','primary'));
    ...
    var hasGrounding=!!(candidate.measured&&candidate.measured.scores&&Number.isFinite(Number(candidate.measured.scores.grounding)));
    if(hasGrounding&&!diag)list.appendChild(modalButton(text('诊断定位','Diagnose grounding'),text('只发送1次定位测试，用于补充旧档案的定位诊断。','Sends one grounding request to repair diagnostic data for an older profile.'),'diagnose'));
