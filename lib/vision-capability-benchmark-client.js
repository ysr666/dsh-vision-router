...
export const CAPABILITY_BENCHMARK_CLIENT = String.raw`(function(){
  ...
  async function openBenchmarkModal(row,control){
    ...
    if(candidate.measured)appendMeasuredDetails(panel,candidate.measured); else { var empty=css(document.createElement('div'),{marginTop:'16px',opacity:'0.72'}); empty.textContent=candidate.imageCapability==='text-only'?text('Host当前将此模型标记为仅文本；该标签只作提示，本次测评仍会实际发送图片验证。','Host currently marks this model text-only; the label is advisory and this benchmark will still send images to verify it.'):text('尚未测评。Auto不会根据模型名称推断能力，暂时保持你的设置顺序。','Not benchmarked. Auto will not infer capability from the model name and keeps your configured order for now.');panel.appendChild(empty); }
