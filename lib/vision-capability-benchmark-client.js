...
export const CAPABILITY_BENCHMARK_CLIENT = String.raw`(function(){
  ...
  function modeText(mode) {
    if (mode === 'full') return text('完整测试', 'full');
    if (mode === 'grounding') return text('定位诊断', 'grounding diagnostic');
    return text('快速测试', 'quick');
  }
  ...
  function resetButtons(control) {
    ...
    setButton(control, 'diagnose', false, text('诊断定位', 'Diagnose grounding'));
  ...
  function renderControl(row, control, body) {
    ...
    if (job && job.state === 'queued') {
      setStatus(control, text('排队中 · 第' + Number(job.position || 1) + '位 · ', 'Queued · #' + Number(job.position || 1) + ' · ') + modeText(job.mode));
      control.dataset.jobId = job.id || '';
    ...
    var needsGroundingDiagnostic = hasGroundingScore && !groundingDiagnostic;
    ...
    if (job && job.state === 'failed') {
      ...
      setStatus(control,
        measuredText + (needsGroundingDiagnostic ? text(' · 定位诊断待补', ' · grounding diagnostic missing') : ''),
        groundingDiagnostic ? groundingDiagnosticText(groundingDiagnostic) : measuredText);
    ...
    if (needsGroundingDiagnostic) {
      setButton(control, 'diagnose', true, text('诊断定位', 'Diagnose grounding'));
    }
  ...
  function makeControl(row) {
    ...
    actions.appendChild(actionButton('diagnose', text('诊断定位', 'Diagnose grounding')));
  ...
  function cloudCostConfirm(candidate, mode) {
    ...
    var count = mode === 'full' ? 6 : mode === 'grounding' ? 1 : 3;
    var label = mode === 'full' ? text('完整测试', 'Full test') : mode === 'grounding' ? text('定位诊断', 'Grounding diagnostic') : text('快速测试', 'Quick test');
    return window.confirm(text(
      label + '会向这个云端模型发送约' + count + '次生成的测试图片请求，可能产生API费用。继续？',
      label + ' sends about ' + count + ' generated-image request' + (count === 1 ? '' : 's') + ' to this cloud model and may incur API charges. Continue?'
    ));
  }
  ...
  async function handleAction(row, control, action) {
    ...
    if (action === 'diagnose') return enqueue(row, control, 'grounding', false);
...
export function installCapabilityBenchmarkClient(ctx) {
  ctx?.inject?.(['webServer'], (webCtx) => {
    webCtx.effect(
      ...
      'vision-router: capability benchmark queue client controls',
    )
  })
}
