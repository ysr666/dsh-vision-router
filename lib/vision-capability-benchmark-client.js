...
export const CAPABILITY_BENCHMARK_CLIENT = String.raw`(function(){
  ...
  function renderControl(row, control, body) {
    ...
    if (job && job.groundingDiagnostic) {
      control.dataset.groundingDiagnostic = JSON.stringify(job.groundingDiagnostic);
    ...
    if (job && job.state === 'failed') {
      ...
      setStatus(control, measuredText, job && job.groundingDiagnostic ? groundingDiagnosticText(job.groundingDiagnostic) : measuredText);
