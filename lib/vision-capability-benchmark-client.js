...
export const CAPABILITY_BENCHMARK_CLIENT = String.raw`(function(){
  ...
  function renderControl(row, control, body){
    ...
    if (background) {
      ...
      var backgroundMsg = text('正在测评','Benchmarking');
      if (backgroundAxis) backgroundMsg += ' · ' + backgroundAxis;
      backgroundMsg += text(' · 后台自动',' · background');
      setStatus(control, backgroundMsg);
      setPrimary(control, text('测评中','Benchmarking'), 'menu', candidate.benchmarkable === true, true);
      return;
    }
  ...
  function hasActiveJobs(body){
    var manual = !!(body&&Array.isArray(body.jobs)&&body.jobs.some(function(job){return job&&(job.state==='queued'||job.state==='running');}));
    var background = body&&body.background;
    return manual || !!(background && (background.active === true || background.running));
  }
