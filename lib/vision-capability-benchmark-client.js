...
export const CAPABILITY_BENCHMARK_CLIENT = String.raw`(function(){
  ...
  function install(){
    ...
    // The DSH shell replaces document.documentElement while booting, which
    // silently detaches an observer installed at head-parse time. Keep the
    // observer attached to the live root and sweep for unadorned chain rows:
    // the native card layout mounts them lazily on disclosure, so without a
    // working observer the benchmark controls and the Auto intro never show.
    var benchObserver=null,benchObserverRoot=null;
    function armBenchObserver(){
      var root=document.documentElement;
      if(!root)return;
      if(benchObserver&&benchObserverRoot===root)return;
      try{if(benchObserver)benchObserver.disconnect();}catch(_){}
      if(typeof MutationObserver!=='function')return;
      benchObserver=new MutationObserver(function(records){for(var i=0;i<records.length;i+=1){var added=records[i]&&records[i].addedNodes;for(var j=0;added&&j<added.length;j+=1){if(nodeTouchesChain(added[j])){scheduleScan(false);return;}}}});
      try{benchObserver.observe(root,{childList:true,subtree:true});benchObserverRoot=root;}catch(_){benchObserver=null;benchObserverRoot=null;}
    }
    armBenchObserver();
    var benchWatchTimer=setInterval(function(){
      if(!benchObserverRoot||!benchObserverRoot.isConnected){armBenchObserver();scheduleScan(false);return;}
      try{
        var rows=document.querySelectorAll(ROW_SELECTOR);
        for(var k=0;k<rows.length;k+=1){
          var row=rows[k];
          if(completeSelection(rowSelection(row))&&!controlFor(row)){scheduleScan(false);return;}
        }
      }catch(_){}
    },1000);
    scheduleScan(true);
