...
export const CAPABILITY_BENCHMARK_CLIENT = String.raw`(function(){
  ...
  async function runBenchmark(row, control) {
    ...
    try {
      ...
      snapshotAt = 0;
      syncRunningControls('');
      // Preserve the initiating row's terminal success/failure state. A forced
      // rescan here used to overwrite a just-rendered error ~50ms later with
      // the default idle copy, making real provider failures only flash once.
      // Other rows are already re-enabled by syncRunningControls('').
    }
  }
