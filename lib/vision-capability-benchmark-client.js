...
export const CAPABILITY_BENCHMARK_CLIENT = String.raw`(function(){
  ...
  function measuredTextOnly(body,key){
    var item=backgroundExcluded(body,key);return !!item&&item.reason==='measured-text-only';
  }
  function backgroundEligible(body,candidate){
    var bg=body&&body.background;
    if(!bg||bg.active!==true||!candidate||candidate.benchmarkable!==true||backgroundExcluded(body,candidate.key))return false;
  ...
  function renderControl(row, control, body){
    ...
    if(measuredTextOnly(body,candidate.key)){
    ...
    if(job&&job.state==='failed'){
      ...
      var waiting=body&&body.background&&body.background.paused
        ...
        : candidate.imageCapability==='text-only'
          ? text('Host标记仅文本 · 等待实际后台测评验证','Host marks text-only · waiting for real background verification')
          : text('等待后台测评 · Auto暂时保持设置顺序','Waiting for background benchmark · Auto keeps configured order for now');
      setStatus(control,waiting);
    }else if(candidate.imageCapability==='text-only')setStatus(control,text('Host标记仅文本 · 仅作提示；可用测试识图/测评实际验证','Host marks text-only · advisory only; Test vision / Benchmark can verify it'));
  ...
  async function openBenchmarkModal(row,control){
    ...
    list.appendChild(modalButton(candidate.measured||isMeasuredTextOnly?text('快速重测','Quick retest'):text('快速测评','Quick benchmark'),text('约3次请求 · 覆盖 OCR 和通用 · 快速建立Auto依据','About 3 requests · covers OCR and general · quickly establishes Auto evidence'),'quick','primary'));
    list.appendChild(modalButton(candidate.measured||isMeasuredTextOnly?text('完整重测','Full retest'):text('完整测评','Full benchmark'),text('约6次请求 · 覆盖结构化、OCR、文档、定位、通用','About 6 requests · covers structured, OCR, document, grounding and general'),'full'));
