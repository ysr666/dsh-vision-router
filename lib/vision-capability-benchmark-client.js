...
export const CAPABILITY_BENCHMARK_CLIENT = String.raw`(function(){
  ...
  function freshnessText(measured){
    if (!measured) return '';
    if (measured.freshness === 'stale') return text('测评已陈旧 · 暂不参与自动选择', 'Benchmark is stale · not used for auto selection');
    if (measured.autoEligible === true || measured.freshness === 'fresh') return text('可用于自动选择', 'Eligible for auto selection');
    return '';
  }
  function measuredMetaText(measured){
    if (!measured) return '';
    return [coverageKindText(measured) + ' · ' + coverageText(measured), ageText(measured.measuredAt), freshnessText(measured)].filter(Boolean).join(' · ');
  }
  function compactMeasuredText(measured){
    var value = coverageKindText(measured) + ' · ' + coverageText(measured);
    var age = ageText(measured && measured.measuredAt); if (age) value += ' · ' + age;
    if (measured && measured.freshness === 'stale') value += text(' · 已陈旧', ' · stale');
    return value;
  }
  ...
  function appendMeasuredDetails(container, measured){
    ...
    var meta=css(document.createElement('div'),{fontSize:'12px',opacity:'0.66',marginTop:'4px'}); meta.textContent=[ageText(measured.measuredAt),freshnessText(measured)].filter(Boolean).join(' · '); card.appendChild(meta);
    var grid=css(document.createElement('div'),{display:'grid',gridTemplateColumns:'minmax(90px,1fr) auto auto',columnGap:'12px',rowGap:'7px',marginTop:'12px',fontSize:'12px'});
  ...
  async function openBenchmarkModal(row,control){
    ...
    var panel=css(document.createElement('div'),{width:'min(460px, calc(100vw - 32px))',maxHeight:'min(700px, calc(100vh - 40px))',overflow:'auto',background:'Canvas',color:'CanvasText',border:'1px solid rgba(127,127,127,.25)',borderRadius:'16px',boxShadow:'0 18px 60px rgba(0,0,0,.22)',padding:'18px'});
