...
export const CAPABILITY_BENCHMARK_CLIENT = String.raw`(function(){
  ...
  function mutationTouchesRows(records) {
    for (var i = 0; i < records.length; i++) {
      ...
      for (var j = 0; j < nodes.length; j++) {
        ...
        try {
          if (typeof node.matches === 'function' && node.matches('.vr-chain-row')) return true;
          if (typeof node.querySelector === 'function' && node.querySelector('.vr-chain-row')) return true;
        } catch (_) {}
      }
    }
    return false;
  ...
  function install() {
    ...
    if (typeof MutationObserver === 'function' && document.documentElement) {
      var observer = new MutationObserver(function(records) {
        if (mutationTouchesRows(records || [])) scheduleScan(false);
      });
