...
const NEW_INSTALL = `  function install() {
    installGuideMutationFence();
    // The root hardening prelude owns the global ModuleLoader lifecycle. Register
    // this presentation patch with it so a create() result or later full-slot
    // replacement receives the same wrapper without duplicating loader state.
    var registry = window.__dshVisionRouterLoaderPatches;
    if (registry && typeof registry.register === 'function') {
      registry.register(patchLoader);
      return;
    }
    // Compatibility fallback when this prelude is embedded on its own.
    var descriptor = Object.getOwnPropertyDescriptor(window, '__ModuleLoader__');
