...
async function convertBlocks(blocks, bytesOf, depth = 0) {
  ...
  if (typeof blocks === 'string') {
    return blocks === '' ? out : [{ type: 'text', text: blocks }]
  }
  ...
  for (const block of blocks) {
    ...
    if (block.type === 'text' && typeof block.text === 'string' && block.text !== '') {
      ...
      // Direct OpenAI-compatible routes already materialize attachments as
      // data-URI image_url blocks. Accept that shape too so switching a local
      // backend to Anthropic format preserves the complete conversation.
