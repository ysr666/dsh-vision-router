...
function currentTextProvider(ctx, fallbackProvider) {
  // The hidden native route is an internal target used while stealth takeover
  // is active. Never turn it back into the public route or the two wrappers
  // would recurse into each other.
  if (fallbackProvider === 'deepseek-official-native') return fallbackProvider
      ...
      if (property === 'stream') {
        ...
        return (options) => {
          ...
          if (store?.pendingDelegate === true && options && typeof options === 'object') {
            ...
            // is itself another Vision Router adapter (stealth/native bridge),
            // its own nested calls must keep their explicit provider.
