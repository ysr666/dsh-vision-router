...
function directProviderFor(candidate, config, core) {
  if (candidate?.provider !== 'vision-http') return undefined
...
async function renderFixturePng(core, fixture) {
  if (typeof core?.loadSharp !== 'function') throw new Error('sharp renderer is unavailable')
  const sharp = await core.loadSharp()
  return sharp(Buffer.from(String(fixture?.svg ?? ''), 'utf8'), { failOn: 'none' }).png().toBuffer()
}
...
async function adapterMessages(ctx, png, fixture) {
  ...
  if (!attachments || typeof attachments.saveImage !== 'function') {
    throw new Error('attachment service is unavailable for exact adapter benchmark')
  }
  ...
  for await (const chunk of stream) {
    ...
    if (chunk?.type === 'finish') {
      ...
      if (kind === 'error' || kind === 'aborted') {
        throw new Error(chunk.reason?.failure?.message ?? kind)
      }
    }
  }
  return text
  ...
  return {
    ...
    async run(key, intents, requestSignal) {
      ...
      if (candidate.benchmarkable !== true || !candidate.endpoint || !candidate.endpointFingerprint) {
        const error = new Error('this backend has no stable endpoint fingerprint and cannot persist exact benchmark evidence')
      ...
      const signal = mergedSignal(requestSignal, controller.signal, AbortSignal.timeout(runTimeoutMs))
      const backend = {
        provider: candidate.provider,
        model: candidate.model,
