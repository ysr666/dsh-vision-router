...
import { normalizeSessionAffinityId } from './session-affinity.js'
import {
...
export function createExactCapabilityInvoker(ctx, core, candidate, config, options = {}) {
  ...
  // Benchmarks are intentionally outside user conversations. Give one entire
  // invoker/run its own stable identity so affinity-required adapters and
  // direct OpenCode Go bridges work without polluting a real conversation or
  // reusing one global fake session across unrelated benchmark runs.
  const benchmarkSessionId =
    normalizeSessionAffinityId(options.sessionId) ??
