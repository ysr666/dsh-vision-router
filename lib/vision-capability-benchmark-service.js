...
export const CAPABILITY_BENCHMARK_MODE_REQUESTS = Object.freeze({ quick: 3, full: 6, grounding: 1 })
...
function tagInvocationError(value) {
  ...
  error.benchmarkClass = classifyCapabilityBenchmarkFailure(error)
  return error
}
...
export function createExactCapabilityInvoker(ctx, core, candidate, config, options = {}) {
  ...
  const prepareFixture = async (fixture) => {
    ...
    const value = candidate.provider === 'vision-http'
      ? { png }
      : { png, messages: await adapterMessages(ctx, png, fixture) }
  ...
  const invoke = async ({ backend, fixture, exactBackend, allowFallback, signal }) => {
    ...
    try {
      const assets = await prepareFixture(fixture)
      ...
      const callExactHttp = async () => {
        ...
        const output = await callDirect(exactHttpProvider, openAIMessages(assets.png, fixture.prompt), {
      ...
      return { output, usedFingerprint: backend.fingerprint, transport, latencyMs }
    } catch (error) {
  ...
  invoke.preflight = async (fixtures = []) => {
    for (const fixture of fixtures) await prepareFixture(fixture)
  }
  return invoke
}

function normalizeMode(value) {
  if (value === 'full' || value === 'grounding') return value
  return 'quick'
}
...
function profileFreshnessSummary(record, coverage, now = Date.now()) {
  if (coverage.length === 0) return 'unknown'
  const states = coverage.map((axis) => capabilityProfileAxisFreshness(record, axis, now))
  const fresh = states.filter((state) => state === 'fresh').length
  const stale = states.filter((state) => state === 'stale').length
  if (fresh === states.length) return 'fresh'
  if (fresh > 0) return 'mixed'
...
async function publicCandidate(ctx, core, config, store, candidate) {
  ...
  const freshnessByAxis = Object.fromEntries(coverage.map((axis) => [axis, capabilityProfileAxisFreshness(record, axis)]))
  const measuredAtByAxis = Object.fromEntries(coverage.map((axis) => [axis, capabilityProfileAxisMeasuredAt(record, axis)]))
  const autoEligibleAxes = coverage.filter((axis) => freshnessByAxis[axis] === 'fresh')
  const staleAxes = coverage.filter((axis) => freshnessByAxis[axis] === 'stale')
  const freshness = record ? profileFreshnessSummary(record, coverage) : undefined
  const measured = record && coverage.length > 0
    ? {
        ...
        freshness,
        freshnessByAxis,
        autoEligible: autoEligibleAxes.length > 0,
        autoEligibleAxes,
        staleAxes,
...
export function createCapabilityBenchmarkManager(ctx, config, core, options = {}) {
  ...
  const executeCandidate = async ({ candidate, current, intents, signal, onProgress }) => {
    ...
    await baseInvoke.preflight?.(listCapabilityBenchmarkFixtures(intents))
    let completed = 0
  ...
  const executeJob = async (job) => {
    ...
    try {
      ...
      let groundingDiagnostic = execution.record?.groundingDiagnostic ?? groundingDiagnosticFromResults(execution.results)
      if (job.mode === 'grounding' && groundingDiagnostic) {
        const existing = await store.get(candidate.endpointFingerprint)
        if (existing) {
          const merged = await store.put({
            ...existing,
            groundingDiagnostic,
          })
          groundingDiagnostic = merged?.groundingDiagnostic ?? groundingDiagnostic
        }
      }
