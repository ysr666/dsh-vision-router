...
const FATAL_FAILURE_CLASSES = new Set([
  'auth',
  'rate-limit',
  'timeout',
  'unsupported-image',
...
function tagFatalInvocationError(value) {
  const error = value instanceof Error ? value : failureError(value)
  const failureClass = classifyCapabilityBenchmarkFailure(error)
  if (FATAL_FAILURE_CLASSES.has(failureClass)) {
    error.benchmarkFatal = true
    error.benchmarkClass = failureClass
  }
  return error
}
  ...
  return async ({ backend, fixture, exactBackend, allowFallback, signal }) => {
    ...
    try {
      ...
      const callExactHttp = async () => {
        ...
        return callDirect(exactHttpProvider, openAIMessages(png, fixture.prompt), {
      ...
      return { output, usedFingerprint: backend.fingerprint, transport }
    } catch (error) {
      throw tagFatalInvocationError(error)
    }
  }
}
