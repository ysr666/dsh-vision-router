...
export function isV2AcceptanceRequestAuthorized(req) {
  return isLocalUiRequest(req) && sameOriginRequest(req)
}
  ...
  ctx?.inject?.(['webServer'], (webCtx) => {
    webCtx.effect(() => webCtx.webServer.register({
      ...
      handler: async (req, res) => {
        ...
        if (!isV2AcceptanceRequestAuthorized(req)) {
          sendJson(res, 403, {
            ok: false,
            code: 'V2_ACCEPTANCE_LOCAL_ONLY',
            error: 'v2 acceptance is available only to same-origin clients on the local DSH machine',
          })
          return
        }
        try {
