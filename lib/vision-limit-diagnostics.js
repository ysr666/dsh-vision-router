...
export function formatVisionTurnGuard(turnBudgetMs) {
  ...
  const limit = seconds === undefined ? '已配置的时间上限' : `${seconds} 秒上限`
  return `Vision Router 本轮视觉${limit}已耗尽。不要再调用视觉工具；请基于已经获得的证据作答，并明确仍存在的不确定性。默认配置为“不限制”；长任务如需继续识图，请检查“设置 → Vision Router → 高级 → 首次识图后的整轮时间上限”。`
}
  ...
  const wrapTool = (def) => {
    ...
    return {
      ...
      async execute(args, exec) {
        ...
        const turn = exec?.agent?.turn ?? exec?.turn
        if (session) {
          const previous = loggedTurns.get(session)
          if (previous === turn) return result
          loggedTurns.set(session, turn)
        }
        const limits = snapshot()
        const startedAt = exec?.agent?.startedAt
        const elapsedMs = Number.isFinite(Number(startedAt)) ? Math.max(0, Date.now() - Number(startedAt)) : undefined
        logger?.warn?.(
          'vision-router: vision turn deadline exhausted turn=%s budgetMs=%d elapsedMs=%s',
          turn === undefined ? 'unknown' : String(turn),
          limits.turnBudgetMs,
