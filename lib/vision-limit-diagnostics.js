  ...
  const wrapTool = (def) => {
    ...
    return {
      ...
      async execute(args, exec) {
        ...
        if (session) {
          const existing = turnState.get(session)
          if (!existing || existing.turn !== turn) {
            turnState.set(session, { turn, startedAt: Date.now(), exhaustionLogged: false })
          }
        }
        const result = await def.execute(args, exec)
        const parsed = parseResult(result)
        if (parsed?.code !== TURN_BUDGET_CODE) return result
        const state = session ? turnState.get(session) : undefined
        if (state?.exhaustionLogged) return result
        if (state) state.exhaustionLogged = true
        const limits = snapshot()
        const elapsedMs = state ? Math.max(0, Date.now() - state.startedAt) : undefined
        logger?.warn?.(
