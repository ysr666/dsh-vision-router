...
export const CAPABILITY_EVIDENCE_CONTRACT_REVISION = 1
...
//
// The explicit contract revision is the non-time-based invalidation seam for
// changes in the exact measurement/wire contract. Commit H adds a per-fixture
// visual proof challenge, so measurements made before this revision must not be
// silently reused even though provider/model/endpoint are unchanged.
export function capabilityEvidenceFingerprint({
  ...
  return capabilityBenchmarkFingerprint({
    ...
    config: {
      ...source,
      visionCapabilityEvidenceContractRevision: CAPABILITY_EVIDENCE_CONTRACT_REVISION,
    },
  })
}
