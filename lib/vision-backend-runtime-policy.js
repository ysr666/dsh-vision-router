...
function recordAcceptance(observer, event) {
  try { observer?.record?.(event) } catch { /* observation is non-authoritative */ }
}
...
function directStream(ctx, core, plan, call, logger, source, acceptanceObserver) {
  return {
    async *[Symbol.asyncIterator]() {
      ...
      recordAcceptance(acceptanceObserver, {
        kind: 'preflight-bridge-attempt',
        provider: call.provider,
        model: call.model,
        backend: `${call.provider}/${call.model}`,
        transport: 'direct-http',
        source,
        reason: 'host-text-projection',
      })
      try {
        ...
        recordAcceptance(acceptanceObserver, {
          kind: 'preflight-bridge-failed',
          provider: call.provider,
          model: call.model,
          backend: `${call.provider}/${call.model}`,
          transport: 'direct-http',
          source,
          code: String(error?.code ?? error?.status ?? error?.name ?? 'ERROR'),
          outcome: 'failed',
        })
        throw error
      }
        ...
        return {
          async *[Symbol.asyncIterator]() {
            ...
            if (delivery === 'text-projected' && (capability.image === true || explicit)) {
              ...
              yield* directStream(ctx, core, plan, call, logger, source, options.acceptanceObserver)
              return
            }
