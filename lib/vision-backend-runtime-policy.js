...
import { blocksHaveRetainedImage, isOffloadedImageBlock, offloadedImagePlaceholder } from './image-offload-compat.js'
...
function contentHasImage(_core, messages) {
    ...
    for (const block of Array.isArray(content) ? content : []) {
      ...
      if (block?.type === 'image' && block.attachment) {
        if (isOffloadedImageBlock(block)) {
          converted.push({ type: 'text', text: offloadedImagePlaceholder(block) })
          continue
        }
