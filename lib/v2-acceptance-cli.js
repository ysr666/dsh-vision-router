...
import { V2_AUTO_EXECUTION_PROBE_PATH } from './vision-capability-shadow.js'
...
export function parseAcceptanceArgs(argv = []) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (value === '--accept-real-execution') { options.acceptedRealExecution = true; continue }
  ...
  if (options.listCandidates && (
    options.acceptedSafeMutations || options.acceptedRealExecution || options.provider || options.acceptedProviderRequests || options.acceptedChargeableCloud || options.force || options.mode !== 'quick'
  ...
  if (!options.provider && !options.acceptedRealExecution && (options.acceptedProviderRequests || options.acceptedChargeableCloud || options.force || options.mode !== 'quick')) {
    throw new Error('provider authorization/mode options require --provider or --accept-real-execution')
  }
  if (options.acceptedRealExecution && (!options.acceptedProviderRequests || !options.acceptedChargeableCloud)) {
    throw new Error('--accept-real-execution requires --allow-provider-requests and --allow-chargeable-cloud')
  }
  if (options.acceptedRealExecution && (options.provider || options.force || options.mode !== 'quick')) {
    throw new Error('--accept-real-execution cannot be combined with --provider, --force, or --mode')
  }
...
function previewExecutionSemantics(body) {
  const expectedActive = body?.routingMode === 'auto'
  return body?.diagnosticReadOnly === true
    && body?.autoPreviewOnly === !expectedActive
    && body?.executionActive === expectedActive
    && body?.executionScope === 'router-owned-visual-tools'
    && body?.executionFailClosed === true
    && body?.healthIncluded === false
}
...
async function inspectLivePublicSurfaces(runtimeUrl) {
  ...
  cases.push(surfaceCase(
    'X-preview',
    preview.ok
      && previewExecutionSemantics(preview.body)
      && previewViolations.length === 0,
    'live routing diagnostics remain read-only, secret-minimized, and report the current Auto execution state accurately',
    {
      status: preview.status,
      routingMode: preview.body?.routingMode ?? null,
      diagnosticReadOnly: preview.body?.diagnosticReadOnly ?? null,
      autoPreviewOnly: preview.body?.autoPreviewOnly ?? null,
      executionActive: preview.body?.executionActive ?? null,
      executionScope: preview.body?.executionScope ?? null,
      executionFailClosed: preview.body?.executionFailClosed ?? null,
      healthIncluded: preview.body?.healthIncluded ?? null,
      violations: previewViolations,
    },
  ))

  const executionProbe = await requestJson(runtimeUrl, V2_AUTO_EXECUTION_PROBE_PATH)
  const probeViolations = executionProbe.ok ? inspectPublicSurfacePayload(executionProbe.body) : [`HTTP ${executionProbe.status}`]
  cases.push(surfaceCase(
    'E01-auto-execution-scope',
    executionProbe.ok
      && executionProbe.body?.ok === true
      && executionProbe.body?.scopeHooked === true
      && executionProbe.body?.transientOverrideWorks === true
      && executionProbe.body?.restored === true
      && executionProbe.body?.providerRequestsMade === 0
      && executionProbe.body?.executionCapable === true
      && executionProbe.body?.executionScope === 'router-owned-visual-tools'
      && executionProbe.body?.executionFailClosed === true
      && probeViolations.length === 0,
    'real DSH core settings scope accepts a process-local Auto order and restores the original view with zero provider requests',
    {
      status: executionProbe.status,
      scopeHooked: executionProbe.body?.scopeHooked ?? null,
      transientOverrideWorks: executionProbe.body?.transientOverrideWorks ?? null,
      restored: executionProbe.body?.restored ?? null,
      providerRequestsMade: executionProbe.body?.providerRequestsMade ?? null,
      executionCapable: executionProbe.body?.executionCapable ?? null,
      executionScope: executionProbe.body?.executionScope ?? null,
      executionFailClosed: executionProbe.body?.executionFailClosed ?? null,
...
function requestedFixtures(mode) {
  const intents = mode === 'full'
    ? undefined
    : mode === 'grounding'
      ? ['grounding']
      : ['ocr', 'general']
  return listCapabilityBenchmarkFixtures(intents)
}

function visualProofFailureDiagnostic(afterSnapshot, key, mode, startedAt) {
  const floor = Math.max(0, Number(startedAt) - 5_000)
  const job = (Array.isArray(afterSnapshot?.jobs) ? afterSnapshot.jobs : [])
    .filter((entry) =>
      entry?.key === key
      && entry?.mode === mode
      && entry?.state === 'failed'
      && entry?.errorCode === 'CAPABILITY_BENCHMARK_VISUAL_PROOF_FAILED'
      && Number(entry?.finishedAt) >= floor)
    .sort((a, b) => Number(b?.finishedAt) - Number(a?.finishedAt))[0]
  const completed = Number(job?.completed)
  const fixture = Number.isInteger(completed) && completed > 0
    ? requestedFixtures(mode)[completed - 1]
    : undefined
  return {
    identified: Boolean(job && fixture),
    failedFixture: fixture?.id ?? null,
    failedIntent: fixture?.intent ?? null,
...
export function inspectProviderEvidence({ beforeSnapshot, afterSnapshot, providerReport, key, mode = 'quick', startedAt = 0 } = {}) {
  ...
  if (providerReport?.ok === true) {
    ...
    const liveCase = Array.isArray(providerReport?.cases)
      ? providerReport.cases.find((entry) => entry?.id === 'B-live')
      : undefined
    if (liveCase?.details?.errorCode === 'CAPABILITY_BENCHMARK_VISUAL_PROOF_FAILED') {
      const diagnostic = visualProofFailureDiagnostic(afterSnapshot, key, mode, startedAt)
      cases.push(surfaceCase(
        'J0B-failure-fixture',
        diagnostic.identified,
        'visual-proof failure identifies the exact generated fixture without exposing the raw model response',
        diagnostic,
      ))
    }
  }
...
function summarize(report) {
  ...
  append('Real Auto execution', report.realExecution?.cases)
...
export async function runAcceptanceCli(argv = process.argv.slice(2), io = console) {
  ...
  const wantsRealExecution = options.acceptedRealExecution === true
  const wantsProvider = typeof options.provider === 'string' && options.provider.trim() !== ''
  if (!wantsSafe && !wantsRealExecution && !wantsProvider) {
    io.error('dsh-vision-router-acceptance: choose --accept-safe-mutations, --accept-real-execution, --provider ... --allow-provider-requests, or --list-candidates')
    io.error(usage().trimEnd())
    return 2
  }
  ...
  if (!options.json) {
    ...
    if (wantsRealExecution) io.log('Real execution phase: temporarily places measured MiniMax/GLM routes first, makes at most two visual provider requests, and restores exact user-layer settings.')
    if (wantsProvider) {
  ...
  if (wantsRealExecution) {
    ...
    realExecution = response.body
    if (!response.ok && realExecution?.ok !== false) {
      realExecution = { ok: false, code: `HTTP_${response.status}`, error: 'real execution acceptance request failed' }
    }
  }
  ...
  const report = {
    ok: (!safe || safe.ok === true)
      && (!realExecution || realExecution.ok === true)
      && surfaces.ok
