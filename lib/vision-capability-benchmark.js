...
export function listCapabilityBenchmarkFixtures(intents = CORE_BENCHMARK_INTENTS) {
  return intents.map((intent) => capabilityBenchmarkFixture(intent))
}
...
export function capabilityBenchmarkFingerprint({ provider, model, endpoint, config } = {}) {
  const safeConfig = config && typeof config === 'object'
    ? Object.fromEntries(Object.entries(config).filter(([key]) => !/key|token|secret|password|authorization/i.test(key)).sort(([a], [b]) => a.localeCompare(b)))
    : undefined
  const payload = JSON.stringify({ provider: String(provider ?? ''), model: String(model ?? ''), endpoint: String(endpoint ?? ''), config: safeConfig ?? null })
