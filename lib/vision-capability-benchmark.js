...
const FIXTURES = Object.freeze({
  ...
  grounding: {
    ...
    prompt: 'Locate the SAVE button. Return only [x1,y1,x2,y2] for the tight button rectangle using coordinates normalized to the 0-1000 range. Do not add prose.',
...
function extractGroundingValue(value) {
  ...
  const number = '-?\\d+(?:\\.\\d+)?'
  const flatArray = new RegExp(`\\[\\s*(${number})\\s*,\\s*(${number})\\s*,\\s*(${number})\\s*,\\s*(${number})\\s*\\]`)
  const arrayMatch = text.match(flatArray)
  if (arrayMatch) {
    return { value: arrayMatch.slice(1, 5).map(Number), source: 'bracket-array' }
  }
