...
function text(x, y, value, size = 28, extra = '') {
  return `<text x="${x}" y="${y}" font-family="Arial,Helvetica,sans-serif" font-size="${size}" fill="#111" ${extra}>${esc(value)}</text>`
}
...
const FIXTURES = Object.freeze({
  structured: {
    ...
    prompt: 'Inspect the image and return ONLY JSON with keys visual_kind, overview, regions, visible_text, relationships, uncertainties. regions and visible_text must be arrays. Include the exact visible tokens STATUS, READY, Queue, 3 jobs, Latency, and 820 ms when they are visible.',
  ...
  grounding: {
    ...
    prompt: `Locate the SAVE button. Return ONLY JSON {"x1":integer,"y1":integer,"x2":integer,"y2":integer} in image pixels for the tight button rectangle. Image size is ${WIDTH}x${HEIGHT}.`,
    expected: { box: { x1: 516, y1: 344, x2: 692, y2: 416 } },
  },
  document: {
    id: 'document-table-v1',
    intent: 'document',
    svg: svgDoc([
    ...
    expected: { tokens: ['Order Summary', 'Camera', '$120', 'Cable', '$15', 'Total', '$135', 'R-4821'] },
  },
  general: {
    id: 'general-scene-v1',
    intent: 'general',
...
export function capabilityBenchmarkFingerprint({ provider, model, endpoint, config } = {}) {
...
function rawBox(value) {
  ...
  for (const key of ['box', 'bbox', 'bounding_box', 'boundingBox']) {
...
export function normalizeGroundingBox(value, width = WIDTH, height = HEIGHT) {
  ...
  let coordinateSpace = 'pixels'
  if (min >= 0 && max <= 1.000001) {
    x1 *= width; x2 *= width; y1 *= height; y2 *= height
    coordinateSpace = 'normalized-1'
  } else if (min >= 0 && max <= 100 && (x2 < width * 0.5 || y2 < height * 0.5)) {
    x1 = x1 / 100 * width; x2 = x2 / 100 * width; y1 = y1 / 100 * height; y2 = y2 / 100 * height
    coordinateSpace = 'percent-100'
  } else if (min >= 0 && max <= 1000 && (x1 > width || x2 > width || y1 > height || y2 > height)) {
...
export function scoreCapabilityBenchmarkResult(fixture, output, latencyMs) {
  ...
  if (intent === 'ocr') {
    ...
    const normalized = normalizeGroundingBox(parsed)
    accuracy = boxIoU(normalized?.box, fixture.expected.box)
    details = {
      iou: accuracy,
      parsed,
      ...
      formatValid: normalized !== undefined,
    }
