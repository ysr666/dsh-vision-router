 ...
 */
export function installPiAiBridgeWireCompat(ctx, logger, acceptanceObserver) {
          ...
          if (patched !== init) {
            ...
            try {
              acceptanceObserver?.record?.({
                kind: 'bridge-wire-compat',
                provider: facts.provider,
                model: facts.model,
                backend: `${facts.provider}/${facts.model}`,
                transport: 'direct-http',
                maxTokensField: facts.maxTokensField ?? 'default',
              })
            } catch {
              // Acceptance observation never affects the request.
            }
