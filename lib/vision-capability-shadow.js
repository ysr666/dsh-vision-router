...
function localAndHttpSources(config, core) {
  ...
  const localByRoute = new Map(
    (Array.isArray(locals) ? locals : [])
      .map((entry) => [routeIdentity(entry), entry])
      .filter(([key]) => key),
  )
...
async function configuredAndLocalCandidates(ctx, config, core) {
  ...
  // The configured chain is the user's primary routing intent. Preserve it
  // exactly, including explicit vision-http rows, before appending other
  // explicitly enabled local/HTTP backends or the fixed built-in fallback tier.
...
function appendHttpCandidates(config, core, out, seen, configuredHttpRoutes, sources) {
  const explicitHttpRoutes = new Set(
    (Array.isArray(config?.httpProviders) ? config.httpProviders : [])
      .map(routeIdentity)
      .filter(Boolean),
  )
...
function usableMeasuredRecord(record, now = Date.now()) {
  const measuredAt = Number(record?.measuredAt)
  return Number.isFinite(measuredAt) && measuredAt > 0 && Math.max(0, Number(now) - measuredAt) <= AUTO_MEASURED_MAX_AGE_MS
    ? record
    : undefined
}
...
async function attachEndpointEvidence(ctx, candidate, store) {
  ...
  const measured = usableMeasuredRecord(await store.get(fingerprint))
  ...
  return {
    ...
    ...(measured ? {
      ...
      medianLatencyMs: measured.medianLatencyMs,
    } : {}),
    latencyMs,
  }
}
...
export async function collectCapabilityShadowCandidates(ctx, config, core, store) {
  ...
  // Deliberately do not append arbitrary DSH-discovered visual models here.
  // Discovery may populate model pickers, but only user-selected/enabled routes
  // plus the fixed system fallback tier are eligible for automatic routing.
...
async function collectCapabilityShadowHealth(candidates, healthForCandidate, context) {
  ...
  for (const candidate of candidates) {
    try {
      ...
      // Health is advisory shadow evidence. A failed observation must never
      // block the v1 tool call or silently turn into an unhealthy verdict.
    }
  }
  return health
}
...
export async function buildCapabilityShadowPlan({
  ...
  return {
    ...
    autoPreviewOrder,
    // Transitional alias retained while existing diagnostics/tests migrate.
            ...
            if (enabled) {
              try {
                ...
                logger?.info?.(
                  ...
                  bounded(plan.autoPreviewOrder.join(' -> '), 1200),
                  bounded(plan.measuredBackends.join(', '), 600),
                  bounded(plan.blockedBackends.join(', '), 600),
                )
              } catch (error) {
 ...
 * order suggestion from user-selected routes and exact benchmark evidence, and
 * logs the preview. It does not reorder, skip, retry or replace any backend and
 * therefore cannot change the v1 fallback walk.
 */
export function installCapabilityShadowRuntime(ctx, config, core, options = {}) {
  ...
  return new Proxy(ctx, {
    get(target, property) {
      if (property === 'tools') {
        return wrapTools(target.tools, target, config, core, store, bootstrapBySession, logger, healthForCandidate)
      }
      const value = Reflect.get(target, property, target)
      return typeof value === 'function' ? value.bind(target) : value
    },
