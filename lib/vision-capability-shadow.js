...
function generatedRoute(provider, config) {
  if (provider === nonEmpty(config?.wrapperRoute) || provider === nonEmpty(config?.chainRoute)) return true
  return typeof provider === 'string' && provider.endsWith('-vision')
}
...
async function configuredAndLocalCandidates(ctx, config, core) {
  ...
  for (const pair of configuredPairs(config)) {
    if (pair.provider === 'vision-http' || generatedRoute(pair.provider, config)) continue
    let available = true
...
async function appendDiscoveredCandidates(ctx, config, core, out, seen) {
  ...
  for (const row of Array.isArray(providers) ? providers : []) {
    ...
    if (provider === undefined || provider === 'vision-http' || generatedRoute(provider, config)) continue
    let listed
    try {
...
function appendHttpCandidates(config, core, out, seen) {
  ...
  for (const entry of Array.isArray(providers) ? providers : []) {
    ...
    addCandidate(out, seen, {
      ...
      provider: 'vision-http',
      model: `${name}/${model}`,
      local: /^https?:\/\/(?:127\.0\.0\.1|localhost|\[::1\])(?::|\/|$)/i.test(String(entry.baseURL ?? '')),
      privacy: /^https?:\/\/(?:127\.0\.0\.1|localhost|\[::1\])(?::|\/|$)/i.test(String(entry.baseURL ?? '')) ? 'local' : 'cloud',
...
async function attachEndpointEvidence(ctx, candidate, store) {
  ...
  if (endpoint === undefined) return candidate
