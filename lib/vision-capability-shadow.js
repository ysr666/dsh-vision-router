...
export async function buildCapabilityShadowPlan({ ctx, config, core, store, toolName, args, bootstrap } = {}) {
  ...
  const ranked = rankVisionCandidates({ intent, strategy: routing.strategy, candidates, measured })
  return {
    intent,
          ...
          async execute(args, exec) {
            ...
            if (enabled) {
              try {
                ...
                logger?.info?.(
                  'vision-router: v2 shadow mode=%s preference=%s intent=%s strategy=%s current=[%s] suggested=[%s] measured=[%s]',
                  plan.routingMode,
