...
function settingsServiceWithAutoExecution(settings) {
  if (!settings || (typeof settings !== 'object' && typeof settings !== 'function')) return settings
  const cached = scopedSettingsServiceCache.get(settings)
  if (cached) return cached
    ...
    get(target, property) {
      if (property === 'register') {
        ...
        return (namespace, ...args) => {
          ...
          return namespace === 'vision-router' ? settingsScopeWithAutoExecution(scope) : scope
        }
      }
      const value = Reflect.get(target, property, target)
      return typeof value === 'function' ? value.bind(target) : value
  ...
  scopedSettingsServiceCache.set(settings, wrapped)
  return wrapped
}

function injectedContextWithAutoExecution(ctx) {
  if (!ctx || (typeof ctx !== 'object' && typeof ctx !== 'function')) return ctx
  return new Proxy(ctx, {
    get(target, property) {
      if (property === 'settings') return settingsServiceWithAutoExecution(Reflect.get(target, property, target))
      const value = Reflect.get(target, property, target)
      return typeof value === 'function' ? value.bind(target) : value
    },
  })
...
function injectWithAutoExecution(target) {
  ...
  return (dependencies, callback, ...rest) => {
    ...
    return inject.call(target, dependencies, (child, ...args) =>
      callback(injectedContextWithAutoExecution(child), ...args), ...rest)
  }
}
...
function wrapTools(tools, ctx, fallbackConfig, core, store, runtimePerformanceStore, bootstrapBySession, logger, healthForCandidate) {
                ...
                if (authorityBeforeExecute.autoSelectionAuthorized && settingsUnchanged) {
                  ...
                  if (executionConfig) {
                    logger?.info?.(
                      ...
                      bounded(configuredPairs(executionConfig).map(candidateKeyForConfiguredPair).filter(Boolean).join(' -> '), 1200),
                    )
                  ...
                  logger?.info?.(
                    ...
                    authorityBeforeExecute.autoSelectionAuthorized ? 'settings-changed' : 'authority-revoked',
                  )
                  result = await executeOriginal()
                }
              } else {
...
export function installCapabilityShadowRuntime(ctx, config, core, options = {}) {
  ...
  return new Proxy(ctx, {
    get(target, property) {
      if (property === 'tools') return wrapTools(target.tools, target, config, core, store, runtimePerformanceStore, bootstrapBySession, logger, healthForCandidate)
