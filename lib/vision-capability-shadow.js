...
function usableMeasuredRecord(record, now = Date.now()) {
  ...
  return Number.isFinite(measuredAt) && measuredAt > 0 && Math.max(0, Number(now) - measuredAt) <= AUTO_MEASURED_MAX_AGE_MS ? record : undefined
}
...
async function attachEndpointEvidence(ctx, candidate, store) {
  ...
  const latencyMs = measured?.latencyMs ?? (latencyValues.length > 0 ? latencyValues.reduce((a, b) => a + b, 0) / latencyValues.length : candidate.latencyMs)
  return {
    ...candidate,
    endpoint,
    endpointConfig,
    ...
    ...(measured ? { measured: measured.scores, measuredAt: measured.measuredAt, medianLatencyMs: measured.medianLatencyMs } : {}),
    latencyMs,
  }
}
