...
const SHADOW_MEASURED_MAX_AGE_MS = 30 * 24 * 60 * 60 * 1000
...
function usableMeasuredRecord(record, now = Date.now()) {
  ...
  return Number.isFinite(measuredAt) && measuredAt > 0 && Math.max(0, Number(now) - measuredAt) <= SHADOW_MEASURED_MAX_AGE_MS
    ? record
    : undefined
}
...
export async function buildCapabilityShadowPlan({
  ...
  const ranked = rankVisionCandidates({ intent, strategy: routing.strategy, candidates, measured, health })
  const healthBackends = Object.keys(health)
  const blockedBackends = healthBackends.filter(
    (key) => health[key]?.circuitOpen === true || health[key]?.rateLimited === true,
  )
  return {
    intent,
 ...
 * not reorder, skip, retry or replace any backend and therefore cannot change the v1 fallback walk.
 */
