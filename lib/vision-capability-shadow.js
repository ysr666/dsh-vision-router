...
function freshMeasuredSlice(record, now = Date.now()) {
  ...
  for (const [axis, rawScore] of Object.entries(record.scores ?? {})) {
    ...
    if (Number.isFinite(measuredAt) && measuredAt > 0) measuredAtByAxis[axis] = measuredAt
    const latency = Number(record?.medianLatencyMs?.[axis])
    if (Number.isFinite(latency) && latency >= 0) medianLatencyMs[axis] = latency
  }
  ...
  const measuredAt = timestamps.length > 0 ? Math.max(...timestamps) : undefined
  const latencyValues = Object.values(medianLatencyMs).filter(Number.isFinite)
...
async function attachEndpointEvidence(ctx, candidate, store) {
  ...
  const credential = await resolveVisionCredential(ctx, endpointCredentialRef)
  ...
  const measured = freshMeasuredSlice(rawRecord)
  const latencyMs = measured?.latencyMs ?? candidate.latencyMs
  return {
    ...candidate,
    endpoint,
    endpointConfig,
