...
// Auto execution authority is intentionally stricter than diagnostics:
    ...
    get(target, property) {
      if (property === 'register') {
        ...
        return (namespace, ...args) => {
          ...
          return settingsScopeWithAutoExecution(scope)
        }
      }
      const value = Reflect.get(target, property, target)
      return typeof value === 'function' ? value.bind(target) : value
...
function wrapTools(tools, ctx, core, store, runtimePerformanceStore, bootstrapBySession, logger, healthForCandidate) {
            ...
            return withVisionRuntimePerformanceScope(def.name, args, async () => {
              ...
              if (authorityAtPlan.autoSelectionAuthorized) {
                try {
                  ...
                  logger?.warn?.('vision-router: v2 auto planning failed: %s', bounded(error?.message ?? error, 400))
                  plan = undefined
                }
              }
...
export function installCapabilityShadowRuntime(ctx, config, core, options = {}) {
  ...
  return new Proxy(ctx, {
    get(target, property) {
      if (property === 'tools') return wrapTools(target.tools, target, core, store, runtimePerformanceStore, bootstrapBySession, logger, healthForCandidate)
