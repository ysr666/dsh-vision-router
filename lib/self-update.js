...
export function profileFromArgv(argv = process.argv) {
  ...
  // The settings card only exists in a Web host. If no explicit profile was
  // supplied, DSH's documented Web lifecycle uses the `web` profile.
  return list.slice(2).includes('web') ? 'web' : 'web'
}
