...
export async function compareRgbaStreams(
  ...
  const gate = Number.isFinite(Number(threshold)) && Number(threshold) >= 0
    ? Number(threshold)
    : 16
  const gridCols = Math.max(1, Math.floor(Number(cols) || 8))
  const gridRows = Math.max(1, Math.floor(Number(rows) || 8))
  ...
  if (readerA.buffer.length > 0 || readerB.buffer.length > 0 || !readerA.done || !readerB.done) {
    // Drain one more time to distinguish harmless stream completion from real
    // trailing data. Exact comparison requires the requested frame size only.
    await Promise.all([readerA.fill(Number.MAX_SAFE_INTEGER), readerB.fill(Number.MAX_SAFE_INTEGER)])
