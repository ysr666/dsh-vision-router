...
function visionPresentationTwinIntended(config, sourceProvider, modelId) {
  ...
  if (config.autoWrapProviders !== false) return true
...
export function projectVisionModeDirectoryState(state, config) {
  ...
  const sourceByHiddenProvider = new Map()
  const visibleGroups = []
  for (const group of state.groups) {
    const sourceProvider = visionPresentationSourceForGroup(state.groups, group, config)
    if (sourceProvider) {
      sourceByHiddenProvider.set(group.id, sourceProvider)
      continue
    }
  ...
  if (visibleGroups.length === state.groups.length && visibleCurrent === current) return state
  return {
    ...state,
