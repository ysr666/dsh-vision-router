...
/**
 * Build the real selection to submit while Vision mode is active.
 *
 * Model selection is deliberately sticky: changing provider/model through the
 * stock picker may change the model inside Vision mode, but it may not turn
 * Vision mode off. Only the dedicated Vision button owns the ON/OFF action.
 * Preserve the incoming selection payload exactly (including the presence or
 * absence of reasoningEffort) and rewrite only provider/model route identity.
 */
function visionPresentationSelectionPlan(state, selection, config) {
  const passthrough = { blocked: false, selection }
  if (!selection || typeof selection !== 'object') return passthrough
  if (!state || typeof state !== 'object' || !Array.isArray(state.groups)) return passthrough
  if (!config || typeof config !== 'object') return passthrough
  const current = state.current
  if (!current || typeof current !== 'object') return passthrough

  const currentGroup = visionPresentationGroup(state.groups, current.provider)
  const currentSourceProvider = visionPresentationSourceForGroup(
    state.groups,
    currentGroup,
    config,
  )
  if (
    ...
    !currentGroup ||
    !visionPresentationHasModel(currentGroup, current.model)
  ) {
    return passthrough
  }

  // Direct selection of a verified wrapper is already in the real routing
  // namespace. Leave it untouched so a wrapper never becomes "-vision-vision".
  const selectedGroup = visionPresentationGroup(state.groups, selection.provider)
  if (
    selectedGroup &&
    visionPresentationHasModel(selectedGroup, selection.model) &&
    visionPresentationSourceForGroup(state.groups, selectedGroup, config)
  ) {
    return passthrough
  }

  // Resolve by exact source provider + model. Never match only by model name:
  // different providers commonly expose models with identical IDs.
  const candidates = state.groups.filter((group) =>
    group &&
    visionPresentationHasModel(group, selection.model) &&
    visionPresentationSourceForGroup(state.groups, group, config) === selection.provider,
  )

  // Fail closed while Vision mode is active. Passing an ordinary route here
  // would silently turn Vision mode off, which is not a model-picker action.
  if (candidates.length !== 1) return { blocked: true, selection }
...
export function mapVisionPresentationSelection(state, selection, config) {
  return visionPresentationSelectionPlan(state, selection, config).selection
}
...
const VISIBILITY_HELPER_SOURCE = [
  ...
  visionPresentationSelectionPlan,
...
export const VISION_MODEL_VISIBILITY_PRELUDE = String.raw`(function(){
  ...
  function wrapStore(store, settings, localState) {
    ...
    function getSnapshot() {
      ...
      var localError = localState && typeof localState.error === 'string'
        ? localState.error
        : null;
      if (
        raw === lastRaw &&
        key === lastConfigKey &&
        localError === lastLocalError &&
        lastProjected !== undefined
      ) return lastProjected;
      lastRaw = raw;
      lastConfigKey = key;
      lastLocalError = localError;
      var projected = projectVisionModeDirectoryState(raw, config);
      if (localError && projected && typeof projected === 'object') {
        projected = Object.assign({}, projected, { status: 'error', error: localError });
      }
  ...
  function wrapDirectory(directory, settings) {
    ...
    var localState = { error: null };
    var store = wrapStore(directory.store, settings, localState);
      ...
      get: function(target, property) {
        ...
        if (property === 'select') {
          ...
          return function(selection) {
            ...
            var plan = visionPresentationSelectionPlan(
              raw,
              selection,
              currentVisionConfig(settings)
            );
            if (plan.blocked) {
              var message = '识图模式已开启，但所选模型没有匹配的 + 自动识图 路由';
              localState.error = message;
              return Promise.reject(new Error(message));
            }
            localState.error = null;
            var args = Array.prototype.slice.call(arguments);
            args[0] = plan.selection;
