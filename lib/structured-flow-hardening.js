...
function inferBranchForTool(name, pending) {
  ...
  // A generic description is intentionally conservative for mixed content: it
  // may finish the last remaining branch, but it cannot silently claim the
  // first of two unrelated branches just because it returned some text.
...
function appendSyntheticGuardOnce(decision, baseMessages, state, message, stripEvidenceFollowups = false) {
  ...
  const id = message?.id
  if (typeof id === 'string' && hasMessageId(messages, id)) {
    state.emittedGuardIds.add(id)
    return stripEvidenceFollowups ? { ...decision, messages } : decision
  }
  // Evidence reminders remain idempotent across repeated no-op pre-steps. Stop
  // guards are different: if the Host rebuilt the message list, reinsert the
  // stop instruction so an exhausted state cannot silently lose its fence.
  if (typeof id === 'string' && state.emittedGuardIds.has(id) && !stripEvidenceFollowups) return decision
    ...
    async execute(args, exec) {
      ...
      return runWithDepthConfig(currentConfig, async () => {
        ...
        if (producedUsableStructuredEvidence(result)) {
          ...
          // Re-arm the current reminder while bounded by a separate non-progress
          // fuse, so unlimited call caps cannot become unlimited failure loops.
          recordNonProgress(state)
          if (!state.nonProgressExhausted) rearmEvidenceGuardAfterAttempt(state)
        }
        return result
