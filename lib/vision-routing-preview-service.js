...
function diagnosticCandidate(entry, currentOrder, rawRecords, axis, now) {
  ...
  return {
    ...
    runtimePerformanceObserved: entry.components?.performanceObserved === true,
...
export const VISION_ROUTING_DIAGNOSTICS_PRELUDE = String.raw`(function(){
  ...
  function candidateRow(grid,c){
    var values=[
      ...
      c.runtimeLatencyMs==null?'—':ms(c.runtimeLatencyMs)
    ];
  ...
  function render(){
    ...
    if(state.body){var p=state.body.policy||{};var note=document.createElement('div');note.className='vr-routing-diag-note';note.textContent=text('公式：','Formula: ')+(p.formula||'—')+' · '+text('换序阈值：','Reorder threshold: ')+score(p.minAdvantage)+' · '+text('Benchmark耗时不当作当前速度；凭据不定义能力身份；测评时间只作记录','Benchmark latency is not current speed; credentials do not define capability identity; measurement age is informational')+' · diag v'+String(state.body.diagnosticVersion||'?');node.appendChild(note);(state.body.previews||[]).forEach(function(item){node.appendChild(detailFor(item));});}
