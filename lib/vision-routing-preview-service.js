...
export const VISION_ROUTING_DIAGNOSTICS_PRELUDE = String.raw`(function(){
  ...
  function evidenceLabel(value){
    if(value==='measured')return text('实测','Measured');
  ...
  function outcomeLabel(check){
    ...
    if(check.outcome==='incomparable')return text('至少一侧没有该轴可比实测证据，禁止跨越','At least one side lacks comparable measured evidence for this axis');
  ...
  function render(){
    ...
    if(state.body){var p=state.body.policy||{};var note=document.createElement('div');note.className='vr-routing-diag-note';note.textContent=text('公式：','Formula: ')+(p.formula||'—')+' · '+text('换序阈值：','Reorder threshold: ')+score(p.minAdvantage)+' · '+text('测评时间：仅作记录，不按天数失效','Measurement age: informational only')+' · diag v'+String(state.body.diagnosticVersion||'?');node.appendChild(note);(state.body.previews||[]).forEach(function(item){node.appendChild(detailFor(item));});}
  ...
  async function refresh(force){
    ...
    try{var response=await fetch(ENDPOINT,{method:'GET',headers:{accept:'application/json'},cache:'no-store'});var body=await response.json().catch(function(){return undefined;});if(!response.ok||!body||body.ok!==true||body.diagnosticVersion!==2)throw new Error(body&&body.error?String(body.error):'HTTP '+response.status);state.body=body;}catch(error){state.error=text('诊断读取失败：','Diagnostic fetch failed: ')+String(error&&error.message||error);}finally{state.loading=false;render();}
  }
  function ensure(){
