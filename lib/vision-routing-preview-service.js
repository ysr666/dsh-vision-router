...
export async function buildVisionRoutingPreview({
  ...
  const executionActive = product.mode === 'auto'
  return {
    ok: true,
    diagnosticVersion: 3,
    ...
    autoPreviewOnly: !executionActive,
    executionActive,
...
export const VISION_ROUTING_DIAGNOSTICS_PRELUDE = String.raw`(function(){
  ...
  function detailFor(item){
    ...
    var order=document.createElement('div');order.className='vr-routing-diag-note';var autoLabel=state.body.executionActive?text('Auto执行顺序：','Auto execution: '):text('Auto预览顺序：','Auto preview: ');order.textContent=text('设置顺序：','Configured: ')+(state.body.currentOrder||[]).join(' → ')+'\n'+autoLabel+(item.order||[]).join(' → ');order.style.whiteSpace='pre-line';d.appendChild(order);
    var grid=document.createElement('div');grid.className='vr-routing-diag-grid';
    [text('设置#','Cfg#'),text('Auto#','Auto#'),text('后端','Backend'),text('能力证据','Capability'),text('该轴分','Score'),text('测评耗时','Bench'),text('运行速度','Runtime')].forEach(function(label){var h=document.createElement('div');h.textContent=label;h.className='vr-routing-diag-muted';grid.appendChild(h);});
  ...
  function render(){
    ...
    if(state.body){
      ...
      var execution=state.body.executionActive?text('；Auto执行已启用，仅影响Router自有视觉工具，异常/撤权回退到设置顺序','; Auto execution is active for router-owned visual tools only; errors/revocation fall back to configured order'):text('；当前为固定顺序，Auto仅预览','; Fixed-order mode: Auto is preview-only');
      note.textContent=text('公式：','Formula: ')+(p.formula||'—')+' · '+text('换序阈值：','Reorder threshold: ')+score(p.minAdvantage)+' · '+text('Benchmark耗时不当作当前速度；凭据不定义能力身份；测评时间只作记录','Benchmark latency is not current speed; credentials do not define capability identity; measurement age is informational')+perf+execution+' · diag v'+String(state.body.diagnosticVersion||'?');node.appendChild(note);(state.body.previews||[]).forEach(function(item){node.appendChild(detailFor(item));});
    }
