...
export const V2_SETTINGS_IA_CLIENT = String.raw`(function(){
  ...
  function sync(){
    ...
    var mode=modeOf(chain,panel),background=backgroundOf(panel),dirty=!!root.querySelector('.vr-ia-savebar');
  ...
  function install(){
    ...
    document.addEventListener('click',function(event){
      ...
      if(root&&root.querySelector('.vr-ia-savebar')){
    ...
    if(typeof MutationObserver==='function'&&document.documentElement){
      ...
      observer.observe(document.documentElement,{childList:true,subtree:true,attributes:true,attributeFilter:['data-vr-routing-mode','data-active','disabled']});
    }
    schedule();
  }
  try{install();}catch(_){}
