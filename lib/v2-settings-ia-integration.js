...
export function injectV2SettingsIaIntegration(html) {
  ...
  // htmlHasScriptMarker deliberately recognizes only exact attributes on real
  // <script> opening tags. Keep the CSS side-effect free and pair it with one
  // no-op script marker so all browser injectors share the same decoy-safe
  // idempotency contract.
  const nodes = `<style>${V2_SETTINGS_IA_STYLE}</style><script ${V2_SETTINGS_IA_MARK}>void 0</script>`
