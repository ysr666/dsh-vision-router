 ...
 *
 * SessionVisionStateStore remains the bounded storage owner. This object is the
 * sole owner of incremental durable-log scan, target-only recovery after
 * bounded-cache eviction, tool-result image surface repair and expired
 * structured guard-stop surface repair.
 *
 * The store is an explicit construction dependency. The index never discovers
 * a process-global current store and never replaces stateStore methods.
 * `routeHandlesImages` is an optional direct-core compatibility classifier used
 * only when no turn-local SessionSurfacePolicy exists.
 */
export function createSessionVisionIndex({
  ...
  if (!stateStore || typeof stateStore !== 'object') {
    throw new TypeError('session vision index requires an explicit state store')
  }
  if (typeof stateStore.lookupAttachment !== 'function') {
    throw new TypeError('session vision index state store requires lookupAttachment')
  }

  const store = stateStore
  const primitiveLookup = store.lookupAttachment.bind(store)
  ...
  const scanEventLogWithStore = (session) => {
    if (!session) return
  ...
  const keepToolResultImages = async (session) => {
    const policy = currentSessionSurfacePolicy(
      typeof config === 'function' ? config() : config,
    )
    if (policy.ownership !== undefined) {
      return policy.ownership === 'native-image' || policy.ownership === 'vision-router-owned'
    }
    if (typeof routeHandlesImages === 'function') {
      try {
        return await routeHandlesImages(session) === true
      } catch {
        return false
      }
    }
  ...
  const repairToolResultSurface = async (session) => {
    ...
    const preserve = await keepToolResultImages(session)
 ...
 * `next()` result. Production passes the index owned by SessionVisionRuntime;
 * direct tests may pass either `options.index` or an explicit `stateStore`.
 */
export function installSessionVisionIndexBoundary(ctx, config, core, options = {}) {
  ...
  const index = options.index ?? createSessionVisionIndex({
    ...
    routeHandlesImages: options.routeHandlesImages,
  })
