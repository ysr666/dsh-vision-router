...
import { isVisionControlPlaneFailureResult } from './vision-result-cache-policy.js'
...
/** Live, byte-bounded LRU for successful vision_describe answers. */
export class LiveDescribeCache {
  ...
  set(key, value, now = Date.now()) {
    // Infrastructure/control-plane failures belong to turn memory and the
    // circuit breaker. Publishing one here would make a repaired credential or
    // recovered backend look broken until the semantic answer TTL expires.
    if (isVisionControlPlaneFailureResult(value)) return false
