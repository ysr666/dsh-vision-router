...
function recordAcceptance(scope, event) {
  try { scope?.acceptanceObserver?.record?.(event) } catch { /* observation is non-authoritative */ }
}
...
function markBridgeAttempt(scope, provider) {
  ...
  recordAcceptance(scope, {
    kind: 'bridge-attempt',
    provider: failure.provider,
    model: failure.model,
    backend: `${failure.provider}/${failure.model}`,
      ...
      return (...args) => {
        ...
        if (
          ...
          if (backend === expectedBackend) {
            ...
            recordAcceptance(scope, {
              kind: 'bridge-failed',
              provider: scope.lastBridge.provider,
              model: scope.lastBridge.model,
              backend: `${scope.lastBridge.provider}/${scope.lastBridge.model}`,
              transport: 'direct-http',
              source: scope.lastBridge.source,
              code: compactDiagnosticDetail(args[2] ?? 'unknown'),
              outcome: 'failed',
            })
          }
        }
            ...
            wrapped = {
              ...
              execute(...args) {
                const scope = {
                  ...
                  diagnosticLogger,
                  acceptanceObserver: options.acceptanceObserver,
