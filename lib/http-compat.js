...
export function parseMaxTokensLimit(detail) {
  ...
  if (!/(?:max[_ -]?tokens?|token[^\n]{0,24}(?:limit|max)|最大[^\n]{0,12}(?:token|输出)|限制数值范围)/i.test(text)) {
    return undefined
  }
  const patterns = [
 ...
 * Layer 3: one safe, evidence-driven retry when the server reports an output
 * token ceiling or explicitly requests max_completion_tokens.
 */
export async function fetchWithOpenAICompatibility(fetchImpl, input, init, context = {}) {
  ...
  const limited = bodyWithTokenLimit(requestBody, advertisedLimit)
  if (limited !== undefined) {
    requestBody = limited
    return fetchImpl(input, { ...init, body: JSON.stringify(requestBody) })
  }

  if (shouldUseMaxCompletionTokens(detail) && requestBody.max_tokens !== undefined) {
    const retryBody = { ...requestBody, max_completion_tokens: requestBody.max_tokens }
    delete retryBody.max_tokens
    return fetchImpl(input, { ...init, body: JSON.stringify(retryBody) })
  }
