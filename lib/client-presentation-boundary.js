...
function visionModeTwinIntended(config, sourceProvider, modelId) {
  if (!config || typeof config !== 'object' || config.autoWrapProviders !== false) return true
  const entries = Array.isArray(config.wrappedProviders) ? config.wrappedProviders : []
  return entries.some((entry) => {
    if (!entry || entry.provider !== sourceProvider) return false
...
export function resolveVisionModePair(groups, current, config = {}) {
  ...
  if (current.provider.endsWith('-vision')) {
    ...
    if (!sourceProvider || !visionModeOwnedTwin(groups, sourceProvider, current.provider, current.model, config)) {
      return { mode: 'unavailable' }
    }
    ...
    ctx.inject(['slots', 'modelDirectories'], function(scope) {
      ...
      function VisionModeToggle(props) {
        ...
        React.useEffect(function(){
          ...
          state.current && state.current.provider,
          state.current && state.current.model,
          state.current && state.current.reasoningEffort
        ]);
