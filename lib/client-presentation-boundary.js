...
const DEFAULT_VISION_MODE_WRAPPER_ROUTE = 'deepseek-vision'
...
function visionModeWrapperRoute(config) {
  const route = config && typeof config.wrapperRoute === 'string' ? config.wrapperRoute.trim() : ''
  return route || DEFAULT_VISION_MODE_WRAPPER_ROUTE
}
 ...
 * an owned Vision Router wrapper means ON, its source route means OFF.
 *
 * DeepSeek is a deliberate special case: the built-in source
 * `deepseek-official` maps to the currently configured `wrapperRoute`
 * (`deepseek-vision` by default), because the Host does not generate a
 * `deepseek-official-vision` twin for a route Vision Router already owns.
 * Other providers keep the generated `<provider>-vision` contract.
 */
export function resolveVisionModePair(groups, current, config = {}) {
  ...
  const wrapperRoute = visionModeWrapperRoute(config)
  ...
  if (current.provider === wrapperRoute) {
    if (!visionModeOwnedWrapper(groups, wrapperRoute, current.model)) {
      return { mode: 'unavailable' }
    }
...
export const CLIENT_PRESENTATION_PRELUDE = String.raw`(function(){
  ...
  var DEFAULT_VISION_MODE_WRAPPER_ROUTE = 'deepseek-vision';
  ...
  function patchVisionModeCopy(namespace, dictionaries) {
    ...
    if (next.zh && typeof next.zh === 'object') {
      next.zh = Object.assign({}, next.zh, {
        quickStartBody: '聊天模型负责思考和回答，识图模型负责看图片。需要识图时，在聊天输入框旁主动开启「识图」；开启后会持续生效，直到你关闭或手动切回普通模型。',
        onboardingStep1Body: '在聊天页右下角选择你平时使用的模型。需要发图片时，在输入框旁主动开启「识图」；不需要时再主动关闭。',
        guideStep1Body: '聊天页输入框旁的模型选择器和「识图」按钮已经被高亮。先选择你平时使用的聊天模型；需要识图时主动开启「识图」，开启后会持续生效，直到你关闭或手动切回普通模型。选好后点击「下一步」。'
      });
    }
    if (next.en && typeof next.en === 'object') {
      next.en = Object.assign({}, next.en, {
        quickStartBody: 'Your chat model handles reasoning and answers; the vision model reads images. Turn on “Vision” beside the composer when you want image understanding. It stays on until you turn it off or manually switch back to a normal model.',
        onboardingStep1Body: 'Choose the model you normally use from the lower-right chat selector. When you need image understanding, explicitly turn on “Vision” beside the composer; turn it off again when you no longer need it.',
        guideStep1Body: 'The chat model selector and “Vision” button beside the composer are highlighted. Choose your normal chat model first, then explicitly turn on “Vision” when you need image understanding. It stays on until you turn it off or manually switch back to a normal model. Click “Next” when done.'
      });
    }
    return next;
  }
  ...
  function installVisionModeToggle(ctx, React) {
    ...
    var en = {
      ...
      failedShort: 'Vision switch failed',
      failed: 'Failed to switch Vision mode: {message}',
    ...
    ctx.inject(['slots', 'modelDirectories'], function(scope) {
      ...
      function VisionModeToggle(props) {
        ...
        var actionError = errorState[0];
        var setActionError = errorState[1];
        ...
        React.useEffect(function(){
          setActionError(null);
        }, [state.current && state.current.provider, state.current && state.current.model]);

        var title = actionError
          ? t('failed', { message: actionError })
          : busy
            ? t('switching')
            : loading
              ? t('loading')
        ...
        return React.createElement('button', {
          ...
          onClick: function() {
            ...
            var pending;
            try {
              pending = directory.select(pair.target);
            } catch (error) {
              setActionError(error && error.message ? error.message : t('failedUnknown'));
              return;
            }
            void Promise.resolve(pending).catch(function(error){
              var latest;
              try {
              ...
              var message = error && error.message
                ? error.message
                : latest && typeof latest.error === 'string' && latest.error !== ''
                  ? latest.error
                  : t('failedUnknown');
          ...
          React.createElement('span', { 'aria-hidden': 'true', style: { fontSize: 13, lineHeight: 1 } }, actionError ? '⚠' : '👁'),
          React.createElement('span', null, actionError ? t('failedShort') : t('label'))
        );
      }
  ...
  function decorateVisionRouterPlugin(plugin, React) {
    ...
    function apply(ctx) {
      ...
      try {
        installVisionModeToggle(decoratedCtx, React);
      } catch (error) {
