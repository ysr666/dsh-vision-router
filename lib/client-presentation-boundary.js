...
export function resolveVisionModePair(groups, current, config = {}) {
  ...
  if (current.provider.endsWith('-vision')) {
    ...
    // A real ordinary provider may itself end in "-vision". If this route is
    // not a verified wrapper, keep resolving it as a source instead of
    // rejecting it solely because of the suffix.
  }
  ...
  function patchVisionModeCopy(namespace, dictionaries) {
    ...
    if (next.zh && typeof next.zh === 'object') {
      next.zh = Object.assign({}, next.zh, {
        ...
        quickStartBody: '先选择你平时使用的聊天模型。需要看图时，点击输入框旁的「识图」；出现 ✓ 表示已开启。开启后会持续生效，切换聊天模型时也会继续保持识图模式；只有再次点击「识图」才会关闭。',
        ...
        guideStep1Body: '高亮的是聊天模型选择器；它左侧就是「识图」按钮。先选择你平时使用的聊天模型；需要看图时点击「识图」，出现 ✓ 表示已开启。开启后切换聊天模型时也会继续保持识图模式；只有再次点击「识图」才会关闭。选好后点击「下一步」。'
      });
    }
    if (next.en && typeof next.en === 'object') {
      next.en = Object.assign({}, next.en, {
        quickStartTitle: 'Chat model + Vision mode',
        quickStartBody: 'Choose the chat model you normally use first. When you need image understanding, click “Vision” beside the composer; a ✓ means it is on. Changing chat models keeps Vision mode on; click “Vision” again to turn it off.',
        ...
        guideStep1Body: 'The highlighted control is the chat model selector; the “Vision” button is immediately to its left. Choose your normal chat model first, then click “Vision” when you need image understanding. A ✓ means it is on. Changing chat models keeps Vision mode on; click “Vision” again to turn it off. Click “Next” when done.'
      });
    }
    return next;
  }
