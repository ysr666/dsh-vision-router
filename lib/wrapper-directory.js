 ...
 * a no-op fallback. When a Host-side ownership oracle is supplied, mere route
 * presence is not enough: this plugin instance must own the wrapper and map it
 * back to official DeepSeek.
 */
export function resolveWrapperDirectoryEntry(ctx, baseConfig = {}, options = {}) {
  ...
  if (
    ownership &&
    typeof ownership.sourceFor === 'function' &&
    ownership.sourceFor(wrapperRoute) !== sourceProvider
  ) {
    return undefined
  }
 ...
 */
export function installWrapperDirectoryAlias(ctx, baseConfig = {}, logger = ctx && ctx.logger, options = {}) {
  const llm = ctx && ctx.llm
  if (
    !llm ||
  ...
  const sync = () => {
    ...
    const next = resolveWrapperDirectoryEntry(ctx, baseConfig, options)
