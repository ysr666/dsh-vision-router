...
export function contextWithTwinImageCapabilityFallback(ctx, options = {}) {
  ...
  const currentMainWrapperRoute = () => {
    try {
      const settings = ctx?.get?.('settings')
      const current = settings?.get?.('vision-router')
      if (nonEmptyString(current?.wrapperRoute)) return current.wrapperRoute
    } catch {
      // Settings can be between generations; the composition-time route stays
      // authoritative until the live namespace becomes readable again.
    }
    return configuredMainWrapperRoute
  }
  ...
  const llmProxy = facadeFor(llm, {
    ...
    registerAdapter(providers, adapter, ...rest) {
      ...
      // lazily once that source is mounted. Main/custom wrapper routes are
      // excluded above, so preserving this lazy lifecycle does not broaden the
      // boundary to foreign plugin registrations.
