...
export function contextWithTwinImageCapabilityFallback(ctx, options = {}) {
  ...
  const currentOwnedRoutes = () => {
    const owned = new Set([configuredMainWrapperRoute, configuredChainRoute])
    try {
      const settings = ctx?.get?.('settings')
      const current = settings?.get?.('vision-router')
      if (nonEmptyString(current?.wrapperRoute)) owned.add(current.wrapperRoute)
      if (nonEmptyString(current?.chainRoute)) owned.add(current.chainRoute)
    } catch {
