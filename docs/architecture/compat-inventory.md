...
- **Feature detection:** explicit transport-aware callers bypass the registry; only compatibility paths that call `currentVisionProviderTransport()` consume the currently installed process/profile transport. The registry never patches `globalThis.fetch`. The transport itself owns every Router-created ProxyAgent through request leases: replacement/clear marks an Agent retired, in-flight work releases the final lease before graceful close, and public plugin cleanup unregisters the registry then calls `transport.dispose()`.
- **Removal condition:** every Router-owned compatibility caller receives `VisionProviderTransport` explicitly, production has zero reads of `currentVisionProviderTransport()`, and the install/release registry can be removed without changing proxy, credential, bounded-body, cancellation, or dispatcher-lifecycle behavior.
- **Tests:** `vision-provider-transport` (including A→B replacement, proxy clear, pending-construction unload and synchronous loader failure), `http-compat`, `catalog-corrections`, P2 Data Boundary provider-transport Node 22/24, Host pack/install smoke.
