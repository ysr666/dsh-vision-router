...
| provider transport / proxy | P2-D moved Router-owned HTTP to `VisionProviderTransport`; H0/H1 proved Host-owned default egress; H2 scopes the remaining explicit DVR override to exact Host-owned visual adapter execution. No supported Host seam currently carries that per-plugin override into an arbitrary adapter request without a compatibility observer | KEEP scoped compatibility boundary; no generic Host route duplication |
