...
> 📌 **公告（v1.3.1）**
>
> **v1.3.1**：识别「未声明图片输入」的视觉模型（用户反馈：智谱官方渠道）——常见多模态命名（glm-4.6v、qwen-vl、gpt-4o 等）在视觉后端下拉自动识别；高级设置「额外视觉模型」两列下拉可强制指定其他模型（如 glm-4.6）；渠道适配器拒绝图片时插件直连该渠道自身的 baseURL + 凭据，无需手改 settings.yaml。
>
