...
# Runtime order: user/native vision models -> configured direct HTTP -> built-in OVH.
sub(
    'index.js',
    r"export function httpProvidersOf\(config, allowDefault = true\) \{.*?\n\}\n\n/\*\*\n \* Drop http providers",
...
sub(
    'index.js',
    r"  const httpProviders = \(\) => \{\n    const raw = httpProvidersOf\(current\(\), current\(\)\.freeFallback !== false\).*?\n  \}\n  const resolveCredential",
    """  const httpProviders = () => {
    const raw = httpProvidersOf(current(), current().freeFallback !== false)
    // User/native adapter-backed vision models are tried before this direct
    // HTTP layer. Configured direct HTTP endpoints stay ahead of the built-in
    // anonymous OVH models, which therefore remain a true final fallback.
...
# Keep vision-http usable internally but publish zero models to the global picker.
sub(
    'index.js',
    r"(    const httpAdapter = \{.*?      providerRetryPolicy\(\) \{\n        return undefined\n      \},)\n      async listModels\(\) \{\n        return httpEntries\.map\(\(entry\) => \(\{.*?\n        \}\)\)\n      \},",
    r"\1\n      async listModels() {\n        return []\n      },",
)

# Settings client gets the fixed fallback metadata without exposing the internal route.
rep(
    'index.js',
...
# Onboarding and explanatory copy.
...
rep(
    ...
    "onboardingBody: '你已有的模型已经自动获得识图入口。聊天页右下角只选择会话/文字模型：发图片时选带「+ 自动识图」的模型组；视觉后端在插件设置里单独配置。',",
)
rep(
    'lib/client.js',
    ...
    "onboardingBody: 'Your existing models now have image-ready entries. The lower-right picker is only for the conversation/text model: choose a group marked “+ Auto Vision” when sending images; vision backends are configured separately in plugin settings.',",
)
rep(
    'lib/client.js',
...
# Never surface vision-http as a provider option; do not blanket-label arbitrary HTTP as free.
...
if text.count(suffix) != 2:
    raise SystemExit(f'expected 2 vision-http group label suffixes, got {text.count(suffix)}')
write('lib/client.js', text.replace(suffix, ''))

text = read('lib/client.js')
text, n = re.subn(r" \+\n\s*\((?:row|pair)\.provider === 'vision-http' \? t\('freeTag'\) : ''\)", '', text)
if n != 3:
    raise SystemExit(f'expected 3 blanket free model labels, got {n}')
write('lib/client.js', text)

# Render the fixed fallback under editable user rows.
rep(
    'lib/client.js',
...
marker = 'httpProvidersOf appends built-in OVH fallback after configured HTTP providers'
...
# README: brain vs eyes, one user model per row, fixed OVH fallback.
rep(
    'README.zh.md',
...
sub(
    'README.md',
    r"The vision chain walks providers in order and only surfaces an error after every one failed:\n\n1\. the \*\*built-in free endpoint\*\*.*?\n2\. configured `httpProviders`.*?;\n3\. configured `providers`.*?\.\n",
