...
test('settings UX keeps engineering controls behind advanced groups', () => {
  ...
  assert.equal(source.includes("networkAuthorityHint: '默认不接管网络出口：Vision Router 沿用 DSH/Host 当前网络路径。下方代理只用于需要给视觉请求单独指定出口的高级覆盖。'"), true)
  assert.equal(source.includes("proxyLabel: '视觉专用代理覆盖'"), true)
  assert.equal(source.includes("proxyLabel: 'Vision-only proxy override'"), true)
