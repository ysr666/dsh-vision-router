...
test('core benchmark suite v2 is generated locally and includes bilingual OCR coverage', () => {
  ...
  assert.deepEqual(fixtures.map((fixture) => fixture.id), [
    'structured-dashboard-v2', 'ocr-latin-ui-v2', 'ocr-zh-chat-v2',
    'grounding-target-v2', 'document-table-v2', 'general-scene-v2',
  ])
...
test('Chinese chat OCR fixture uses the same exact-order scorer', () => {
  ...
  const missingQuote = scoreCapabilityBenchmarkResult(fixture, fixture.expected.text.replace('引用：先别合并\n', ''), 620)
  assert.equal(exact.score, 1)
...
test('document scorer requires valid JSON, row order, pairing, total and order id', () => {
  ...
  const exact = scoreCapabilityBenchmarkResult(fixture, JSON.stringify({
    title: 'Order Summary', rows: [{ item: 'Camera', amount: '$120' }, { item: 'Cable', amount: '$15' }], total: '$135', order_id: 'R-4821',
  }), 400)
  const swapped = scoreCapabilityBenchmarkResult(fixture, JSON.stringify({
    title: 'Order Summary', rows: [{ item: 'Cable', amount: '$15' }, { item: 'Camera', amount: '$120' }], total: '$135', order_id: 'R-4821',
  }), 400)
  const tokenSoup = scoreCapabilityBenchmarkResult(fixture, 'Order Summary Camera $120 Cable $15 Total $135 R-4821', 400)
  assert.equal(exact.score, 1)
...
test('grounding scorer normalizes common 0-1, percentage and 0-1000 coordinate spaces before IoU', () => {
  ...
  const ratio = { x1: box.x1 / 768, y1: box.y1 / 512, x2: box.x2 / 768, y2: box.y2 / 512 }
  const percent = { x1: box.x1 / 768 * 100, y1: box.y1 / 512 * 100, x2: box.x2 / 768 * 100, y2: box.y2 / 512 * 100 }
  const thousand = { x1: box.x1 / 768 * 1000, y1: box.y1 / 512 * 1000, x2: box.x2 / 768 * 1000, y2: box.y2 / 512 * 1000 }
...
test('grounding scorer accepts GLM-style prose and bracket coordinates in 0-1000 space', () => {
  const fixture = capabilityBenchmarkFixture('grounding')
  const scored = scoreCapabilityBenchmarkResult(fixture, 'The position of the SAVE button is [672,672,901,813].', 300)
  assert.ok(scored.score > 0.99)
...
test('grounding scorer accepts native GLM box markers and tuple wrappers', () => {
  const fixture = capabilityBenchmarkFixture('grounding')
  const outputs = [
    '<|begin_of_box|><672,672,901,813><|end_of_box|>', '(672,672,901,813)', '<672,672,901,813>', '672,672,901,813',
  ]
  for (const output of outputs) {
    const scored = scoreCapabilityBenchmarkResult(fixture, output, 300)
    assert.ok(scored.score > 0.99, output)
...
test('structured scorer requires both schema coverage and visible evidence', () => {
  ...
  const good = scoreCapabilityBenchmarkResult(fixture, JSON.stringify({
    visual_kind: 'ui', overview: 'dashboard', regions: ['sidebar', 'status'],
    visible_text: ['STATUS', 'READY', 'Queue', '3 jobs', 'Latency', '820 ms'],
    relationships: ['sidebar left of cards'], uncertainties: [],
  }), 800)
...
test('capability identity is secret-safe, binds deployment/suite, and ignores credential rotation', () => {
  const common = {
    provider: ' p ',
    model: ' m ',
  ...
  const a = capabilityEvidenceFingerprint({ ...common, credentialFingerprint: 'cred_aaa' })
  const sameSanitized = capabilityEvidenceFingerprint({
    provider: 'p', model: 'm', endpoint: 'https://a.test/v1?a=1&b=2&token=SECRET-2',
    config: {
  ...
  const changedPath = capabilityEvidenceFingerprint({ ...common, endpoint: 'https://a.test/v2?a=1&b=2' })
  const changedNestedConfig = capabilityEvidenceFingerprint({ ...common, config: { temperature: 0, transport: { timeoutMs: 9000 } } })
  const changedModel = capabilityEvidenceFingerprint({ ...common, model: 'm2' })
...
test('aggregate averages multiple fixtures for the same intent and reports Benchmark observation latency', () => {
