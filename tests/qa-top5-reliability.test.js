...
test('vision tool runtime resolves bare relative paths from the active session cwd and forwards cancellation to fs', async () => {
...
test('cancelled vision work leaves the image resource queue before a slot becomes available', async () => {
  ...
  releaseFirst()
  await new Promise((resolve) => setImmediate(resolve))
  assert.equal(started, false, 'cancelled queued work must never enter the heavyweight operation')
})

test('multi-image vision_describe bypasses the legacy order-insensitive cache while single-image calls keep it', async () => {
  const harness = runtimeHarness({ scopeValue: { cache: true, cacheMaxEntries: 200 } })
  let scope
  harness.wrapped.inject(['settings'], (child) => {
    scope = child.settings.register('vision-router', {}, {})
  })
  harness.wrapped.tools.register({
  ...
  assert.equal(single.cache, true)
  const multi = await harness.registered.execute({ attachmentIds: ['A', 'B'] }, exec)
  assert.equal(multi.cache, false)
  assert.equal(multi.cacheMaxEntries, 200)
  assert.equal(scope.get().cache, true, 'cache projection must be invocation-local, not a settings mutation')
})
...
test('attachment admission migration re-applies to replacement attachment services', () => {
  ...
  replacementCallback({
    get(name) { return name === 'attachments' ? second : undefined },
  })
...
function rc8ClientHarness() {
    ...
    async call(channel, endpoint, payload) {
      ...
      if (endpoint === 'describe') {
        return {
          ok: true,
          value: remoteEnabled
            ? { enabled: true, reason: 'enabled', writable: true, view: { value: { routing: false }, user: {}, revision } }
        ...
        async json() {
          return {
            ...
            value: {
              ...
              present: payload.operation === 'set',
              ...(payload.operation === 'set' ? { value: payload.value } : {}),
              revision,
            },
          }
        },
  ...
  const snapshot = {
    ...
    mode: 'host',
    revision: 7,
    value: { allowRemoteSettings: false },
    user: {},
  }
  const rawScope = {
...
test('rc8 queue-to-live loader transition preserves both local permission and remote risk wrappers', async () => {
  ...
  const result = await connection.rpc.call('/vision-router-settings', 'describe', {})
