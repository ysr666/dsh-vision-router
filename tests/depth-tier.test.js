...
test('integration: explicit call caps and structured x>=1 each have one runtime owner', () => {
  ...
  assert.equal(hardening.includes('return state.postBootstrapEvidenceCalls >= 1 ? 0 : 1'), true)
  assert.equal(hardening.includes('state.successfulEvidenceCalls += 1'), true)
  assert.equal(hardening.includes('if (state.bootstrapDone) state.postBootstrapEvidenceCalls += 1'), true)
