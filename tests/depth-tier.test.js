...
test('client.js integration: custom depth lives in the canonical editor shared by both entry points', () => {
  ...
  assert.equal(client.includes('留空或填 0 = 不限制深挖次数。'), true)
  assert.equal(client.includes('Empty or 0 = no cap.'), true)
  assert.equal(client.includes('（视同标准档）'), false)
  assert.equal(client.includes('(same as Standard)'), false)
